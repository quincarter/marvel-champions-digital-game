...
import { appendCardHistory, cardHistoryOf, emptyCardHistoryLog, eventRefs, type CardHistoryLog } from "./card-history.js";

async function playedGame(): Promise<{ history: CardHistoryLog; state: GameState; viewer: PlayerId; allEvents: readonly GameEvent[] }> {
...
describe("card history", () => {
  ...
  test("every card drawn this game shows up in its own history as drawn", () => {
    const drawn = played.allEvents.find((event): event is Extract<GameEvent, { type: "cardDrawn" }> => event.type === "cardDrawn");
  ...
  test("a played card's history says so, in the same words the table log uses", () => {
    const played1 = played.allEvents.find((event): event is Extract<GameEvent, { type: "cardPlayed" }> => event.type === "cardPlayed");
  ...
  test("round tags are 'round.beat', two-digit beat, non-decreasing within a card's own history", () => {
    const anyId = played.allEvents.find((e): e is Extract<GameEvent, { type: "cardDrawn" }> => e.type === "cardDrawn")!.instanceId;
  ...
  test("a card with no events naming it has no history", () => {
    expect(cardHistoryOf(played.history, "no-such-instance" as InstanceId, played.state, played.viewer, POOL_DEPS)).toEqual([]);
  });
...
describe("eventRefs", () => {
  test("names both sides of damage dealt with a known source", () => {
    const refs = eventRefs({ type: "damageDealt", targetInstanceId: "t" as InstanceId, sourceInstanceId: "s" as InstanceId, amount: 3 });
    expect(refs).toEqual(["t", "s"]);
  });

  test("names only the target when a damage source is not tracked", () => {
    const refs = eventRefs({ type: "damageDealt", targetInstanceId: "t" as InstanceId, sourceInstanceId: null, amount: 3 });
