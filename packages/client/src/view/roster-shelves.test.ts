...
const packName = (code: string): string => ({ core: "Core Set", gob: "Green Goblin", twc: "The Wrecking Crew" })[code] ?? code;

function candidate(item: Item, packCode: string | null, extra: Partial<ShelfCandidate<Item>> = {}): ShelfCandidate<Item> {
...
describe("shelvesOf", () => {
  ...
  test("null packCode buckets into 'Your decks', first, only when non-empty", () => {
    const withDecks = shelvesOf([candidate(item("u", "Ultron"), "core"), candidate(item("d", "My deck"), null)], packOrder, packName, "");
  ...
  test("a chip-excluded candidate never appears on any shelf", () => {
    const shelves = shelvesOf([candidate(item("u", "Ultron"), "core", { passesChips: false })], packOrder, packName, "");
    expect(shelves).toEqual([]);
  });

  test("empty query keeps everything that passes chips", () => {
    const shelves = shelvesOf([candidate(item("u", "Ultron"), "core"), candidate(item("r", "Rhino"), "core")], packOrder, packName, "");
  ...
  test("a nested item match narrows a shelf without dropping the whole thing", () => {
    const shelves = shelvesOf([candidate(item("u", "Ultron"), "core"), candidate(item("r", "Rhino"), "core")], packOrder, packName, "ultr");
  ...
  test("a query matching a pack's own name keeps the whole shelf, even items whose own fields don't match", () => {
    const shelves = shelvesOf([candidate(item("u", "Ultron"), "core"), candidate(item("r", "Rhino"), "core")], packOrder, packName, "core set");
    expect(shelves).toHaveLength(1);
    expect(shelves[0]!.items.map((i) => i.id)).toEqual(["u", "r"]);
  });

  test("a pack-name match never pulls in 'Your decks' — it has no pack name to match", () => {
    const shelves = shelvesOf([candidate(item("d", "My deck"), null, { searchHaystacks: ["My deck"] })], packOrder, packName, "core");
    expect(shelves).toEqual([]);
  });
  ...
  test("accent- and case-insensitive, like the rest of S8's search", () => {
    const shelves = shelvesOf([candidate(item("c", "Café scenario"), "core", { searchHaystacks: ["Café scenario"] })], packOrder, packName, "cafe");
    expect(shelves).toHaveLength(1);
  });
...
describe("flattenShelves", () => {
  test("every item, in shelf order then item order", () => {
    const shelves = shelvesOf(
      [candidate(item("d", "My deck"), null), candidate(item("u", "Ultron"), "core"), candidate(item("g", "Goblin scenario"), "gob")],
      packOrder,
      packName,
      "",
    );
