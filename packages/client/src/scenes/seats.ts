...
/**
 * W1's Deck check scene (docs/phase4-screen-gaps.md §3 W1) lands in a
 * parallel change. Until it exists, "Deck check ▸" routes straight to Table
 * setup — flip `DECK_CHECK_ENABLED` and this function once it does.
 */
const DECK_CHECK_ENABLED = false;
function goToDeckCheckOrTableSetup(scene: Phaser.Scene, draft: SetupDraft): void {
  if (DECK_CHECK_ENABLED) {
    // scene.scene.start(SCENES.deckCheck, { draft });
    return;
  }
...
export class SeatsScene extends Phaser.Scene {
  ...
  #rebuild(): void {
    ...
    const deckCheck = (): void => goToDeckCheckOrTableSetup(this, this.#draft);
