 ...
 * play through the campaigns. One "Unlock everything" switch, then a scrolling list of every wave (with what opens
 * it) and every hero in it, each hero with its own switch.
 *
 ...
 * tap through `setUnlockPrefs`, which every picker reads the next time it draws.
 */
...
export class UnlocksOverlay extends Phaser.Scene {
  ...
  #toggleAll(): void {
    ...
    setUnlockPrefs({ ...prefs, unlockAll: !prefs.unlockAll });
    this.#draw();
  }

  #tap(row: UnlockListRow): void {
    const next = prefsAfterTap(unlocks().prefs, row);
    if (!next) return;
    setUnlockPrefs(next);
    this.#draw();
  }
  ...
  #draw(): void {
    ...
    const headingY = allRect.y + allRect.height + 12;
    label(this, body.x + 16, headingY, "Heroes by wave", typeRole.label, surface.paper.hex, ink.secondary);
    const listRect: Rect = {
      x: body.x + 16,
      y: headingY + 24,
      width: rowWidth,
      height: Math.max(ROW_HEIGHT, body.y + body.height - (headingY + 24) - 16),
    };
  ...
  #renderRow(row: UnlockListRow, rect: Rect): VirtualListRow {
    ...
    const title = this.add
      .text(rect.x + 12, midY, row.title, textStyle(typeRole.body, surface.paper.hex, 0.9))
      .setOrigin(0, 0.5)
      .setFontSize(14);
    const toggleable = heroToggleable(row.state);
    const on = row.state !== "off";
    const pill: Rect = {
      x: rect.x + rect.width - GUTTER - PILL_WIDTH,
      y: midY - PILL_HEIGHT / 2,
      width: PILL_WIDTH,
      height: PILL_HEIGHT,
    };
    const g = this.add.graphics();
    if (on && toggleable) g.fillStyle(surface.paper.hex, 1).fillRect(pill.x, pill.y, pill.width, pill.height);
    g.lineStyle(1.5, surface.paper.hex, toggleable ? 1 : 0.4).strokeRect(pill.x, pill.y, pill.width, pill.height);
    const pillText = label(
      this,
      pill.x + pill.width / 2,
      midY,
      heroStateLabel(row.state),
      typeRole.label,
      on && toggleable ? surface.ink.hex : surface.paper.hex,
      toggleable ? 1 : 0.55,
    ).setOrigin(0.5, 0.5);
    objects.push(title, g, pillText);
    return { objects };
  }
}
