...
/**
 * A real game test for Gamora's own stand-in deck (docs/phase7-wave3-scripting.md "Test conventions", modeled on
 * `../gmw/e2e.test.ts`): Gamora (`support.ts`'s hand-built deck — she has no real precon yet, docs/phase7-wave3.md
 ...
 * deep-equal final state. This exists to catch a Gamora ability that's individually well-tested but breaks when
 ...
 */
test("Rhino (standard), solo: Gamora", () => {
  const config = gamoraScenario("rhino", { seed: 2026 });
  const created = createGame(config, WAVE3_DEPS);
  if (!created.ok) throw new Error(`setup failed: ${created.error.message}`);
  const result = playToOutcome(created.state, WAVE3_DEPS);
  console.info(
    `[wave3 e2e] Rhino (standard) — Gamora: ${result.outcome ? `${result.outcome.result} (${result.outcome.reason})` : "no outcome"} in round ${result.rounds}, ${result.commands} commands`,
  );
