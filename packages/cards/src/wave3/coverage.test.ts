...
const KNOWN_SKIPPED: Readonly<Record<string, readonly string[]>> = {
  gmw: [
    ...
    // (Groot, `gmw/groot-kit.ts`), 16032/16033/16052 (Rocket, `gmw/rocket-kit.ts`), 16020/16048 (Flora and Fauna,
    // scripted identically in both files), 16060.when-revealed (Drang III, `gmw/badoon.ts`),
    // 16073b.the-grand-collection-action (`gmw/museum.ts`), and 16085a.this-way plus 16085b's three refs
    // (`gmw/escape-the-museum.ts`) — every ref that pass closed. `gmw/nebula.ts` itself has no genuine primitive
    // gaps (module docblock) — every primitive it needed had already landed. Ronan the Accuser (16102+, except
    // the Band of Badoon-shared 16117-16121 already scripted) and the Kree Militants/Galactic Artifacts/Badoon
    // Headhunter modular sets are not yet reached. Regenerated with `MC_REFS_PACKS=gmw pnpm refs` (docs/card-
    // scripting-process.md) — never hand-typed.
