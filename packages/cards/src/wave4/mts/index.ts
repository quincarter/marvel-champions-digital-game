 ...
 * `KNOWN_SKIPPED.mts` in `../coverage.test.ts`). Ebony Maw (the box's first scenario) is scripted in `ebony-maw.ts`;
 * Thanos (the box's third scenario) and its own Infinity Gauntlet modular set are scripted in `thanos.ts` and
 * `infinity-gauntlet.ts`.
 */
