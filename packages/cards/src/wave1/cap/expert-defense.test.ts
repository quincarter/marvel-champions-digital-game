...
function protectionCap() {
  ...
  const protection = wave1StarterDeckSetup("msm-protection").deck.filter(
    (id) => !aspectOf.get(id)?.startsWith("hero:"),
  );
  return {
    identityCardId: cap.identityCardId,
    aspects: ["protection"] as const,
    deck: [...signature, ...protection, "03033" as CardId],
  };
}
