...
import { endTurn, firstLegal, inst, instancesOf, moveToHand, P1, payWith, play, playerOf, settle, stackEncounterDeck } from "../../testing/harness.js";
...
describe("Clash of the Titans (Hulk's nemesis set)", () => {
  it("makes the enemy with the highest ATK attack the ally with the highest ATK when no hero is in play", () => {
    ...
    const withBrawn = settle(runHlk(given.state, play(P1, brawn, payWith(given.state, P1, 3, [brawn]))), firstLegal, undefined, HLK_DEPS);
    ...
    const after = settle(runHlk(stacked, endTurn()), firstLegal, (s) => activeEncounterDeck(s).discard.includes(instancesOf(s, "10028")[0]!), HLK_DEPS);
