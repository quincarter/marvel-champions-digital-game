...
const msmVsRhino = () => startMsmGame(wave1Scenario("rhino", { players: [{ starterDeckId: "msm-protection" }], seed: 13 }));
 ...
 */
function putMinionInPlay(state: GameState, player: PlayerId, code: string): { readonly state: GameState; readonly id: InstanceId } {
  ...
  return {
    ...
    state: {
      ...
      players: state.players.map((p) =>
        p.playerId === player ? { ...p, setAside: p.setAside.filter((x) => x !== id), playArea: [...p.playArea, id] } : p,
      ),
      instances: { ...state.instances, [id]: { ...state.instances[id]!, faceup: true, engagedWith: player, controllerId: null } },
    },
  };
}
...
function stackFromSetAside(state: GameState, player: PlayerId, code: string): GameState {
  ...
  return {
    ...state,
    players: state.players.map((p) => (p.playerId === player ? { ...p, setAside: p.setAside.filter((x) => x !== id) } : p)),
...
describe("Home by Dawn (Ms. Marvel's obligation)", () => {
  ...
  it("discards a controlled Persona support (no surge); with none, gains surge and discards itself", () => {
    ...
    const withAamir = (() => {
      ...
      return { state: settle(runMsm(given.state, play(P1, aamir, payWith(given.state, P1, 1, [aamir]))), firstLegal, undefined, MSM_DEPS), aamir };
    })();
    ...
    const after = settle(answer(atChoice, ["1"], MSM_DEPS), (s) => (s.pendingChoice?.prompt.kind === "chooseCards" ? picking(withAamir.aamir)(s) : firstLegal(s)), undefined, MSM_DEPS);
...
describe("Ms. Marvel's nemesis set", () => {
  it("Thomas Edison: takes damage normally while alone (not engaged with another minion)", () => {
    ...
    const after = settle(runMsm(withEdison, play(P1, bigHands, payWith(withEdison, P1, 2, [bigHands]))), picking(edison), undefined, MSM_DEPS);
  ...
  it("Thomas Edison: cannot take damage while you are engaged with another minion", () => {
    ...
    const after = settle(runMsm(withBoth, play(P1, bigHands, payWith(withBoth, P1, 2, [bigHands]))), picking(edison), undefined, MSM_DEPS);
  ...
  it("Harvest: exhausts each Persona support, heals the villain 1 per support exhausted this way", () => {
    ...
    const hero = runMsm(given.state, toHero());
    const withAamir = settle(runMsm(hero, play(P1, aamir, payWith(hero, P1, 1, [aamir, bigHands]))), firstLegal, undefined, MSM_DEPS);
    const villain = activeVillain(withAamir).instanceId;
    const damaged = settle(runMsm(withAamir, play(P1, bigHands, payWith(withAamir, P1, 2, [bigHands]))), firstLegal, undefined, MSM_DEPS);
