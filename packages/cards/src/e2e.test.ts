...
function playAndReplay(label: string, config: GameSetupConfig): void {
  ...
  console.info(
    `[e2e] ${label}: ${outcome ? `${outcome.result} (${outcome.reason})` : "no outcome"} in round ${result.rounds}, ${result.commands} commands`,
  );
...
test("Rhino (standard, Bomb Scare), solo: Spider-Man (Justice)", () => {
  playAndReplay(
    ...
    coreScenario("rhino", { players: seats("core-spider-man-justice"), seed: 2026 }),
  );
}, 120_000);

test("Rhino (expert, Bomb Scare), solo: Iron Man (Aggression)", () => {
  playAndReplay(
    "Rhino (expert) — Iron Man",
    coreScenario("rhino", { difficulty: "expert", players: seats("core-iron-man-aggression"), seed: 404 }),
  );
}, 120_000);
