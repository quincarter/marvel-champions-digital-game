...
export const BOMB_SCARE_SET = defineAbilities({
  ...
  "01110.when-revealed": whenRevealed(
    chooseOne(option("Take 2 damage", takeDamage(2)), option("Place 1 threat on the main scheme", placeThreat(1, theMainScheme))),
  ),
  ...
  "01111.when-revealed": whenRevealed(ifThen(inPlay(BOMB_SCARE), assignDamage(threatOn(named(BOMB_SCARE)), query(["hero", "ally"])), surge())),
