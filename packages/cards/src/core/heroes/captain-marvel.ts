...
export const CAPTAIN_MARVEL_KIT = defineAbilities({
  ...
  "01015.alpha-flight-station-action": action(
    { cost: [exhaustThis, discardFromHandCost(1, 1)] },
    draw(ifElse(youAreCarolDanvers, 2, 1)),
  ),
  ...
  "01017.cosmic-flight-interrupt": heroInterrupt(
...
export const CAPTAIN_MARVEL_OBLIGATION = defineAbilities({
  ...
  "01175.obligation": obligation("Carol Danvers", {
    label: "You are stunned; this card gains surge",
    effects: [stun(yourIdentity), surge()],
  }),
});
...
export const CAPTAIN_MARVEL_NEMESIS = defineAbilities({
  ...
  "01177.yon-rogg-forced-response": forcedResponse(
    after.enemyAttacks("self"),
    placeThreat(1, named(cardName("01176"))),
  ),
