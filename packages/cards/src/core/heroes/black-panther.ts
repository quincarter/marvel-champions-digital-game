...
export const BLACK_PANTHER_KIT = defineAbilities({
  ...
  "01040b.foresight": setup(chooseCards("upgrade", zone("deck", you, { filter: BLACK_PANTHER_UPGRADE }), { min: 1, max: 1 }), moveCards(cards(chosen("upgrade")), "hand"), shuffleDeck()),
  ...
  "01042.ancestral-knowledge-action": alterEgoAction(chooseCards("cards", zone("discard"), { min: 0, max: 3, distinctNames: true }), moveCards(cards(chosen("cards")), "deckShuffle")),
  ...
  "01049.vibranium-suit-special": special({ label: "attack" }, attackAnEnemy(ifElse(finalStep, 2, 1), { moveDamageFrom: yourIdentity })),
});
...
export const BLACK_PANTHER_OBLIGATION = defineAbilities({
  ...
  "01155.obligation": obligation("T'Challa", {
    ...
    effects: [ifThen(exists(YOUR_BLACK_PANTHER_UPGRADES), [chooseTarget("upgrade", YOUR_BLACK_PANTHER_UPGRADES), discard(chosen("upgrade"))])],
  }),
});
...
export const BLACK_PANTHER_NEMESIS = defineAbilities({
  ...
  "01157.killmonger-constant": constant(rule({ kind: "cannotTakeDamage", target: { self: true }, fromSource: BLACK_PANTHER_UPGRADE })),
  ...
  "01159.when-revealed": whenRevealed(
    ...
    chooseOne(option("Deal X damage to your hero", takeDamage(RITUAL_X)), option("Place X threat on the main scheme", placeThreat(RITUAL_X, theMainScheme))),
  ),
});
