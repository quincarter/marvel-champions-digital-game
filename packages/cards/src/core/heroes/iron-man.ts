...
export const IRON_MAN_KIT = defineAbilities({
  ...
  "01029a.iron-man-constant": constant(gets("handSize", scaled(countOf(query("upgrade", { controller: "you", trait: TRAIT.TECH })), { max: 6 }), { self: true })),
  ...
  "01030.war-machine-action": action({ cost: [exhaustThis, damageThisCardCost(2)] }, dealDamage(1, each(query("enemy")))),
  ...
  "01034.stark-tower-action": alterEgoAction(
    { cost: exhaustThis },
    choosePlayer(),
    moveCards(zone("discard", chosenPlayer(), { filter: query("upgrade", { trait: TRAIT.TECH }), topmostOnly: true }), "hand"),
  ),
  ...
  "01037.mark-v-helmet-action": heroAction({ label: "thwart", cost: exhaustThis }, ifThen(aerial, thwart(1, each(query("scheme"))), thwartAScheme(1))),
  ...
  "01038.powered-gauntlets-action": heroAction({ label: "attack", cost: exhaustThis }, attackAnEnemy(ifElse(aerial, 2, 1))),
  ...
  "01039.rocket-boots-action": heroAction({ cost: [exhaustThis, spend({ mental: 1 })] }, gainTraitUntil(TRAIT.AERIAL, yourIdentity, "endOfPhase")),
});
...
export const IRON_MAN_OBLIGATION = defineAbilities({
  ...
  "01170.obligation": obligation("Tony Stark", { label: "Exhaust each upgrade you control", effects: [exhaust(each(query("upgrade", { controller: "you" })))] }),
});
...
export const IRON_MAN_NEMESIS = defineAbilities({
  ...
  "01174.when-revealed": whenRevealed(
    forEachPlayer(eachPlayer, moveCards(topOfDeck(5, thatPlayer), "discard", "backlash"), takeDamage(varOf("backlash.energy"), thatPlayer)),
  ),
});
