...
export const PROTECTION = defineAbilities({
  ...
  "01075.black-widow-interrupt": interrupt(when.encounterCardRevealed(), { cost: [exhaustThis, spend({ mental: 1 })] }, cancelRevealedCard(), revealEncounterCard(you)),
  ...
  "01077.counter-punch-response": response(after.defends(YOUR_HERO), { label: "attack" }, attack(statOf(yourIdentity, "atk"), eventSource)),
  ...
  "01078.get-behind-me-interrupt": heroInterrupt(when.encounterCardRevealed(query("treachery")), cancelWhenRevealed(), enemyAttack(theVillain, { against: you })),
  ...
  "01080.med-team-action": action({ cost: [exhaustThis, removeCounter("medical")] }, chooseTarget("character", FRIENDLY_CHARACTER), heal(2, chosen("character"))),
