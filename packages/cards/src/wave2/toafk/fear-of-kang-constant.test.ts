...
function withObligationInPlay(state: GameState, code: string, owner: PlayerId): GameState {
  ...
  return {
    ...
    players: state.players.map((p) =>
      p.playerId === owner ? { ...p, deck: p.deck.filter((x) => x !== id), playArea: [...p.playArea, id] } : p,
    ),
  };
}
...
const BARE_RULE_DEPS = {
  ...
  abilities: {
    ...
    "11049.fear-of-kang-constant": constant(
      rule({ kind: "cannotAttack", target: query("villain", { name: cardName("11001") }) }),
    ),
  },
};
...
describe("cannotAttack scoping (RuleSpec, see kang-encounter-set.ts's 11049 note)", () => {
  it("a bare, target-only rule blocks the OTHER player's attacks too (Distracting Taunts' own shape)", () => {
    ...
    const attack = {
      ...
      playerId: P2,
      attackerInstanceId: identityOf(state, P2),
    ...
    expect(
      withRule.ok,
      "P2 should not have been blocked by P1's own obligation, but a target-only cannotAttack rule blocked them anyway",
    ).toBe(false);
  });
  ...
  it("a bare, target-only rule also blocks the actual obligation holder (both readings agree on this half)", () => {
    ...
    expect(
      applyCommand(state, attack, NEUTRAL_DEPS).ok,
      "expected P1's attack on Kang to be legal with no cannotAttack rule active",
    ).toBe(true);
    expect(applyCommand(state, attack, BARE_RULE_DEPS).ok, "P1, who does hold Fear of Kang, should be blocked").toBe(
      false,
    );
  });
  ...
  it("the shipped 11049.fear-of-kang-constant (player: you) blocks its own controller but not the other player", () => {
    ...
    const p1Attack = {
      type: "basicAttack" as const,
      playerId: P1,
      attackerInstanceId: identityOf(p1State, P1),
      targetInstanceId: kang,
    };
    expect(
      applyCommand(p1State, p1Attack, WAVE2_DEPS).ok,
      "P1, who controls Fear of Kang, should be blocked from attacking Kang",
    ).toBe(false);
    ...
    const p2Attack = {
      type: "basicAttack" as const,
      playerId: P2,
      attackerInstanceId: identityOf(p2State, P2),
      targetInstanceId: kang,
    };
    expect(
