...
test("Rhino (standard), solo: Ant-Man (Leadership)", () => {
  ...
  console.info(
    `[wave2 e2e] Rhino (standard) — Ant-Man: ${result.outcome ? `${result.outcome.result} (${result.outcome.reason})` : "no outcome"} in round ${result.rounds}, ${result.commands} commands`,
  );
