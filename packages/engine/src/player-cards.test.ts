...
import { ALLY, giveCards, newGame, newGameAtMulligan, resolvePending, RESOURCE, runWith, settle, settleUntil } from "./testing/scenario.js";
...
const use = (id: InstanceId, abilityId: string, player: PlayerId = p1): Command => ({ type: "useAbility", playerId: player, cardInstanceId: id, abilityId: abilityId as never, payment: [] });
...
function setup({ cards = [], abilities = [], players = 1, encounter, identity }: Setup) {
  ...
  const state = newGame({
    ...
    deck: [...playerCards.flatMap((c) => copies(c.id)), ...copies(ENERGY.id), ...copies(MENTAL.id), ...copies(RESOURCE.id, 8), ...copies(ALLY.id)],
...
function stackDeck(state: GameState, player: PlayerId, ...cards: readonly CardId[]): { state: GameState; ids: readonly InstanceId[] } {
  ...
  return {
    ids: top,
    state: { ...state, players: state.players.map((p) => (p.playerId === player ? { ...p, deck: [...top, ...deck] } : p)) },
  };
}
...
describe("deck and discard manipulation", () => {
  it("'discard the top 2 cards of your deck; add each with a printed [mental] resource to your hand' (Black Cat)", () => {
    const cat = stubAbility("black-cat", def({
      trigger: { kind: "response", forced: true, on: { on: "cardEntersPlay", selfIs: "target" } },
      effects: [
        { kind: "moveCards", cards: { kind: "zone", zone: "deck", player: controller, top: { kind: "const", value: 2 } }, to: "discard", bind: "d" },
        { kind: "moveCards", cards: { kind: "ref", ref: { kind: "slot", slot: "d" }, filter: { printedResource: "mental" } }, to: "hand" },
      ],
    }));
  ...
  it("'Deal 1 damage and discard the top 5; +2 damage for each printed [energy] resource' (Repulsor Blast)", () => {
    const blast = stubAbility("repulsor", def({
      trigger: { kind: "action", form: "hero" },
      label: ["attack"],
      effects: [
        { kind: "moveCards", cards: { kind: "zone", zone: "deck", player: controller, top: { kind: "const", value: 5 } }, to: "discard", bind: "d" },
        { kind: "attack", target: { kind: "villain" }, amount: { kind: "scaled", value: { kind: "var", name: "d.energy" }, times: 2, plus: 1 } },
      ],
    }));
  ...
  it("'Look at the top 3 cards of your deck. Add 1 to your hand and discard the others' (Tony Stark)", () => {
    const futurist = stubAbility("futurist", def({
      trigger: { kind: "action" },
      effects: [
        { kind: "chooseCards", slot: "pick", chooser: controller, min: 1, max: 1, from: { kind: "zone", zone: "deck", player: controller, top: { kind: "const", value: 3 } } },
        { kind: "moveCards", cards: { kind: "ref", ref: { kind: "slot", slot: "pick" } }, to: "hand" },
        { kind: "moveCards", cards: { kind: "zone", zone: "deck", player: controller, top: { kind: "const", value: 2 } }, to: "discard" },
      ],
    }));
    ...
    const atChoice = runWith(deps, stacked.state, play(given.ids[0] as InstanceId), use(given.ids[0] as InstanceId, "futurist"));
  ...
  it("'Search your deck for an upgrade and add it to your hand. Shuffle your deck.' (Shuri)", () => {
    const shuri = stubAbility("shuri", def({
      trigger: { kind: "response", forced: true, on: { on: "cardEntersPlay", selfIs: "target" } },
      effects: [
        { kind: "chooseCards", slot: "u", chooser: controller, min: 1, max: 1, from: { kind: "zone", zone: "deck", player: controller, filter: { categories: ["upgrade"] } } },
    ...
    const result = applyCommand(atChoice, { type: "resolveChoice", playerId: p1, choiceId: atChoice.pendingChoice?.choiceId as never, selectedOptionIds: [options[0]?.optionId as string] }, deps);
  ...
  it("'Choose a player. That player returns the topmost Tech upgrade in their discard pile to their hand' (Stark Tower)", () => {
    const tower = stubAbility("stark-tower", def({
      trigger: { kind: "action", form: "alterEgo" },
      effects: [
        { kind: "choosePlayer", slot: "pl", chooser: controller },
        { kind: "moveCards", cards: { kind: "zone", zone: "discard", player: { kind: "slot", slot: "pl" }, filter: { categories: ["upgrade"], trait: TECH }, topmostOnly: true }, to: "hand" },
      ],
    }));
    ...
    const p2Gadgets = mustPlayer(given.state, p2).deck.filter((id) => given.state.instances[id]?.cardId === GADGET.id).slice(0, 2) as [InstanceId, InstanceId];
    const withDiscard: GameState = {
      ...given.state,
      players: given.state.players.map((p) =>
        p.playerId === p2 ? { ...p, deck: p.deck.filter((id) => !p2Gadgets.includes(id)), discard: [...p2Gadgets, ...p.discard] } : p,
      ),
    };
    const atChoice = runWith(deps, withDiscard, play(given.ids[0] as InstanceId), use(given.ids[0] as InstanceId, "stark-tower"));
  ...
  it("'Choose up to 3 different cards in your discard pile and shuffle them into your deck' (Ancestral Knowledge)", () => {
    const knowledge = stubAbility("ancestral", def({
      trigger: { kind: "action", form: "alterEgo" },
      effects: [
        { kind: "chooseCards", slot: "c", chooser: controller, min: 0, max: 3, distinctNames: true, from: { kind: "zone", zone: "discard", player: controller } },
    ...
    const discarded: GameState = {
      ...given.state,
      players: given.state.players.map((p) =>
        p.playerId === p1 ? { ...p, hand: p.hand.filter((id) => ![e1, e2, m1, r1].includes(id)), discard: [e1, e2, m1, r1] } : p,
      ),
    };
  ...
  it("'Return Hellcat to your hand' — a card leaving play to hand loses its damage", () => {
    const hellcat = stubAbility("hellcat", def({ trigger: { kind: "action" }, effects: [{ kind: "moveCards", cards: { kind: "ref", ref: { kind: "self" } }, to: "hand" }] }));
    ...
    const hurt: GameState = { ...inPlay, instances: { ...inPlay.instances, [id]: { ...mustInstance(inPlay, id), damage: 2 } } };
...
describe("form, choices, players", () => {
  it("'Change your form. Then, draw up to your printed hand size' — doesn't use the round's form change; 'after you change to this form' fires", () => {
    const split = stubAbility("split", def({
      trigger: { kind: "action" },
      effects: [{ kind: "changeForm", player: controller }, { kind: "drawUpTo", player: controller, amount: { kind: "handSize", player: controller, printed: true } }],
    }));
    const lift = stubAbility("do-you-even-lift", def({
    ...
    const identity = stubIdentity({ id: "she-hulk", hp: 15, atk: 3, thw: 1, def: 2, rec: 5, heroHandSize: 8, alterEgoHandSize: 6, heroAbilities: [lift.ref] });
      ...
      effects: [
        {
          ...
          chooser: controller,
          options: [
            { label: "Remove 2 threat from a scheme", condition: { kind: "exists", query: { categories: ["sideScheme"], hasThreat: true } }, effects: [] },
            { label: "Draw 3 cards", effects: [{ kind: "draw", player: controller, amount: { kind: "const", value: 3 } }] },
            { label: "Deal 4 damage to an enemy", effects: [{ kind: "dealDamage", target: { kind: "villain" }, amount: { kind: "const", value: 4 } }] },
          ],
        },
      ],
    }));
  ...
  it("'Choose a player to draw 1 card' (Carol Danvers)", () => {
    const commander = stubAbility("commander", def({ trigger: { kind: "action" }, effects: [{ kind: "choosePlayer", slot: "pl", chooser: controller }, { kind: "draw", player: { kind: "slot", slot: "pl" }, amount: { kind: "const", value: 1 } }] }));
    ...
    const atChoice = runWith(deps, given.state, play(given.ids[0] as InstanceId), use(given.ids[0] as InstanceId, "commander"));
  ...
  it("'Each player discards the top 5 cards of their deck; for each printed [energy] resource, that player takes 1 damage'", () => {
    const backlash = stubAbility("backlash", def({
      trigger: { kind: "action" },
      effects: [
        {
          kind: "forEachPlayer",
          players: { kind: "each" },
          effects: [
            { kind: "moveCards", cards: { kind: "zone", zone: "deck", player: { kind: "scoped" }, top: { kind: "const", value: 5 } }, to: "discard", bind: "d" },
            { kind: "dealDamage", target: { kind: "identityOf", player: { kind: "scoped" } }, amount: { kind: "var", name: "d.energy" } },
          ],
        },
      ],
    }));
  ...
  it("'Deal 1 damage to each enemy' (Ground Stomp) — `each` targets", () => {
    const stomp = stubAbility("stomp", def({ trigger: { kind: "action", form: "hero" }, effects: [{ kind: "dealDamage", target: { kind: "each", query: { categories: ["enemy"] } }, amount: { kind: "const", value: 1 } }] }));
...
describe("Special abilities in a sequence (Wakanda Forever!)", () => {
  const claws = stubAbility("claws", def({
    trigger: { kind: "special" },
    label: ["attack"],
    effects: [{ kind: "attack", target: { kind: "villain" }, amount: { kind: "conditional", if: { kind: "varAtLeast", name: "sequence.final", amount: 1 }, then: { kind: "const", value: 4 }, else: { kind: "const", value: 2 } } }],
  }));
  const genius = stubAbility("tactical", def({
    trigger: { kind: "special" },
    label: ["thwart"],
    effects: [{ kind: "thwart", target: { kind: "mainScheme" }, amount: { kind: "conditional", if: { kind: "varAtLeast", name: "sequence.final", amount: 1 }, then: { kind: "const", value: 2 }, else: { kind: "const", value: 1 } } }],
  }));
  const wakanda = stubAbility("wakanda", def({ trigger: { kind: "action", form: "hero" }, effects: [{ kind: "resolveSpecials", cards: { categories: ["upgrade"], controller: "you", trait: BLACK_PANTHER } }] }));
...
describe("play restrictions and upgrade hosts (schema PlayRestrictions / AttachmentHost)", () => {
  it("'Hero form only', 'Max 1 per player', 'Attach to an ally. Max 1 per ally.'", () => {
    ...
    const INSPIRED: AnyCard = { ...stubUpgrade({ id: "inspired", cost: 0 }), attachesTo: { kind: "ally" }, playRestrictions: { maxPerHost: 1 } };
    ...
    const [heroicId, m1, m2, i1, i2, palId] = given.ids as [InstanceId, InstanceId, InstanceId, InstanceId, InstanceId, InstanceId];
    ...
    expect(rejected(deps, withPal, play(i1, { attachToInstanceId: mustPlayer(withPal, p1).identity.instanceId }))).toBe("no_valid_target");
  ...
  it("'Play under any player's control. Your hero gets +1 ATK.' (Combat Training)", () => {
    const training = stubAbility("training", def({ trigger: { kind: "constant", modifiers: [{ stat: "atk", amount: 1, target: { categories: ["hero"], controller: "you" } }] }, effects: [] }));
    const TRAINING: AnyCard = { ...stubUpgrade({ id: "training", cost: 0, abilities: [training.ref] }), playRestrictions: { anyPlayerControl: true, maxPerPlayer: 1 } };
  ...
  it("an identity 'Setup:' ability resolves during setup (T'Challa: search for a Black Panther upgrade)", () => {
    const foresight = stubAbility("foresight", def({
      trigger: { kind: "setup" },
      effects: [
        { kind: "chooseCards", slot: "u", chooser: controller, min: 1, max: 1, from: { kind: "zone", zone: "deck", player: controller, filter: { categories: ["upgrade"], trait: BLACK_PANTHER } } },
        { kind: "moveCards", cards: { kind: "ref", ref: { kind: "slot", slot: "u" } }, to: "hand" },
        { kind: "shuffleDeck", player: controller },
      ],
    }));
    const identity = stubIdentity({ id: "tchalla", hp: 11, atk: 2, thw: 2, def: 2, rec: 4, heroHandSize: 5, alterEgoHandSize: 6, alterEgoAbilities: [foresight.ref] });
...
test("card manipulation, choices and sequences replay to an identical state", () => {
  const futurist = stubAbility("futurist", def({
    ...
    effects: [
      ...
      { kind: "moveCards", cards: { kind: "ref", ref: { kind: "slot", slot: "pick" } }, to: "hand" },
      { kind: "moveCards", cards: { kind: "zone", zone: "deck", player: controller, top: { kind: "const", value: 2 } }, to: "discard", bind: "d" },
      { kind: "chooseOne", chooser: controller, options: [{ label: "draw", effects: [{ kind: "draw", player: controller, amount: { kind: "const", value: 1 } }] }, { label: "shuffle", effects: [{ kind: "shuffleDeck", player: controller }] }] },
    ],
  }));
  const DESK = stubSupport({ id: "desk", cost: 0, abilities: [futurist.ref] });
  const { deps, state } = setup({ cards: [DESK], abilities: [futurist] });
  ...
  while (session.state.pendingChoice) {
    ...
    apply({ type: "resolveChoice", playerId: choice.playerId, choiceId: choice.choiceId, selectedOptionIds: [choice.options[choice.options.length - 1]?.optionId as string] });
  }
