...
import { abilityRefsOf, requiredIdentitySet, unscriptedCards, validateDeck, type DeckProblem, type DeckProblemCode } from "./deck.js";
  ...
  it("is exactly the identity-set cards each Core starter deck lists, at their set quantities", () => {
    ...
    for (const starter of CORE_STARTER_DECKS) {
      ...
      const signature = starter.cards
        .filter(({ cardId }) => (CORE_CARDS.find((card) => card.id === cardId) as { aspect?: string } | undefined)?.aspect === `hero:${identity.id as string}`)
...
const without = (deck: DeckContents, id: string): DeckContents => ({ ...deck, cards: deck.cards.filter((e) => e.cardId !== id) });
...
const codesOf = (deck: DeckContents, pool?: readonly AnyCard[]): readonly DeckProblemCode[] => problemsOf(deck, pool).map((p) => p.code);
...
const synthetic = (from: PlayerCard, overrides: Partial<Omit<PlayerCard, "type" | "aspect">> & { readonly aspect?: Aspect | "campaign" }): PlayerCard =>
  ({ ...from, ...overrides }) as PlayerCard;
...
describe("validateDeck: each problem code, from real Core data", () => {
  ...
  it("identity-set cards are held to their exact set quantity, not the three-copy rule (Black Panther's 5 Wakanda Forever!)", () => {
    const wakanda = CORE_CARDS.filter(isPlayer).filter((c) => c.aspect === "hero:01040a" && c.name === "Wakanda Forever!");
  ...
  it("unique_match: a unique card matching the identity (the identity counts)", () => {
    const peter = synthetic(coreWhere((c) => c.id === "01083"), { id: cardId("x-peter"), name: "Peter Parker" });
  ...
  it("aspect_choice: none, Basic, the same aspect twice, or two for a one-aspect hero", () => {
    ...
    expect(problem({ ...starter(), aspects: ["justice", "justice"] }, "aspect_choice").message).toContain("more than once");
    expect(problem({ ...starter(), aspects: ["justice", "aggression"] }, "aspect_choice").message).toContain("Justice and Aggression");
  });
...
describe("validateDeck: synthetic cards for rules no Core card exercises", () => {
  it("linked_card: a linked card cannot be in a deck, and is not a required part of its identity set", () => {
    ...
    const signatureLinked = synthetic(basicEvent, { id: cardId("x-sig-linked"), aspect: `hero:${SPIDER_MAN}`, keywords: [linked] });
    expect(problemsOf(starter(), [...CORE_CARDS, signatureLinked])).toEqual([]);
  });

  it("permanent cards do not count toward deck size", () => {
    const permanent = synthetic(basicEvent, { id: cardId("x-permanent"), name: "Permanent Thing", keywords: [{ name: "permanent" }] });
  ...
  it("missing_card_data: no valid deck limit, and a Team-Up with no names", () => {
    ...
    expect(problem(withCard(starter(), "x-bad", 1), "missing_card_data", [...CORE_CARDS, bad]).message).toContain("Bad Data");
    const nameless = synthetic(basicEvent, { id: cardId("x-teamup"), name: "Nameless Team-Up", keywords: [{ name: "teamUp" }] });
    expect(problem(withCard(starter(), "x-teamup", 1), "missing_card_data", [...CORE_CARDS, nameless]).message).toContain("Nameless Team-Up");
  });

  it("team_up_identity: a Team-Up card for other heroes; accepted when the identity is named", () => {
    const antWasp = synthetic(basicEvent, { id: cardId("x-antwasp"), name: "Team Effort", keywords: [{ name: "teamUp", names: ["Ant-Man", "Wasp"] }] });
    ...
    const withPeter = synthetic(basicEvent, { id: cardId("x-peterwasp"), name: "Web Effort", keywords: [{ name: "teamUp", names: ["Peter Parker", "Wasp"] }] });
...
describe("validateDeck: identity deckbuilding requirements are data, not card names", () => {
  ...
  it("aspectCount: two aspects are required, and one is refused", () => {
    ...
    expect(problemsOf(spiderManBuilt(["justice", "aggression"], { justice: 7, aggression: 7, basic: 11 }), pool)).toEqual([]);
  });
  ...
  it("equalCardsPerAspect (FAQ Jessica Drew): 7 and 7 is legal, 8 and 6 is not", () => {
    ...
    expect(problemsOf(spiderManBuilt(["justice", "aggression"], { justice: 7, aggression: 7, basic: 11 }), pool)).toEqual([]);
    const p = problem(spiderManBuilt(["justice", "aggression"], { justice: 8, aggression: 6, basic: 11 }), "deckbuilding_requirement", pool);
    expect(p.message).toContain("8 Justice and 6 Aggression");
  });

  it("offAspectPackages (FAQ Maria Hill): all of exactly N titles, or none", () => {
    const support = coreWhere((c) => c.type === "support" && (c.aspect === "aggression" || c.aspect === "leadership" || c.aspect === "protection") && c.traits.length > 0);
...
describe("validateDeck: an identity's separate deck (the Invocation deck)", () => {
  ...
  it("unscriptedCards names separate-deck cards whose abilities are not registered, since setup brings them in", () => {
    const everyCoreRef: AbilityRegistry = Object.fromEntries(CORE_CARDS.flatMap(abilityRefsOf).map((ref) => [ref.id, {} as AbilityDefinition]));
    expect(unscriptedCards(starter(), pool, { abilities: everyCoreRef })).toEqual(["x-inv-1", "x-inv-2"]);
    const withSpells: AbilityRegistry = { ...everyCoreRef, "x-inv-1.special": {} as AbilityDefinition, "x-inv-2.special": {} as AbilityDefinition };
