...
const play = (id: InstanceId, attachTo: InstanceId | null = null, payment: Command extends never ? never : readonly { fromHand: InstanceId }[] = []): Command => ({
  type: "playCard",
  playerId: p1,
  cardInstanceId: id,
  payment,
...
const SCHEME = (acceleration = 0) =>
  stubMainScheme({ id: "scheme", stages: [{ startingThreat: flat(5), targetThreat: flat(40), acceleration: flat(acceleration) }] });
...
const blastAbility = stubAbility("blast", def({
  trigger: { kind: "action" },
  label: ["attack"],
  effects: [
...
describe("prevention (RRG 'Prevent')", () => {
  it("'prevent all of that damage' (Backflip) — a (defense) interrupt to attack damage", () => {
    const backflip = stubAbility("backflip", def({
      trigger: { kind: "interrupt", forced: false, on: { on: "dealDamage", fromAttack: true, targetIs: { categories: ["identity"], controller: "you" } } },
      label: ["defense"],
    ...
    const after = settle(playFromWindow(deps, resolvePending(atDefense, ["decline"], deps), given.ids[0] as InstanceId), undefined, deps);
  ...
  it("'discard Cosmic Flight → prevent 3 of that damage'", () => {
    const flight = stubAbility("cosmic-flight", def({
      trigger: { kind: "interrupt", forced: false, form: "hero", on: { on: "dealDamage", targetIs: { categories: ["identity"], controller: "you" } } },
      label: ["defense"],
  ...
  it("'When threat would be placed on a scheme, prevent 1 of that threat' (limit once per round)", () => {
    const iObject = stubAbility("i-object", def({
    ...
    const identity = stubIdentity({ id: "walters", hp: 15, atk: 3, thw: 1, def: 2, rec: 5, heroHandSize: 4, alterEgoHandSize: 6, alterEgoAbilities: [iObject.ref] });
  ...
  it("'When the villain schemes, reduce the amount of threat placed by 1' (Emergency)", () => {
    const emergency = stubAbility("emergency", def({
    ...
    const after = settle(playFromWindow(deps, runWith(deps, given.state, endTurn), given.ids[0] as InstanceId), undefined, deps);
...
describe("replacement effects (RRG 'Replacement Effect', '\"Instead\"')", () => {
  it("'When threat would be placed on a scheme, you take it as damage instead' (Great Responsibility)", () => {
    const great = stubAbility("great-responsibility", def({
      trigger: { kind: "interrupt", forced: false, form: "hero", on: { on: "placeThreat" } },
      effects: [{ kind: "replaceTriggeringEvent", with: [{ kind: "dealDamage", target: { kind: "identityOf", player: { kind: "controller" } }, amount: { kind: "eventAmount" } }] }],
    }));
  ...
  it("'When any amount of damage would be dealt to Rhino, place it here instead; at 5+ discard this' (Armored Rhino Suit)", () => {
    const suit = stubAbility("rhino-suit", def({
      ...
      effects: [
        {
          ...
          with: [
            ...
            { kind: "if", condition: { kind: "damagedAtLeast", of: { kind: "self" }, amount: 5 }, then: [{ kind: "discardFromPlay", target: { kind: "self" } }] },
          ],
        },
      ],
    }));
    const SUIT = stubAttachment({ id: "suit", attachesTo: { kind: "villain" }, keywords: [{ name: "setup" }], abilities: [suit.ref] });
  ...
  it("'When attached minion would be defeated, heal all damage from it instead, then discard this card' (Biomechanical Upgrades)", () => {
    const bio = stubAbility("bio", def({
      trigger: { kind: "interrupt", forced: true, on: { on: "characterDefeated", targetIs: { hostOfSelf: true } } },
      effects: [{ kind: "replaceTriggeringEvent", with: [{ kind: "heal", target: { kind: "host" }, amount: { kind: "const", value: 99 } }, { kind: "discardFromPlay", target: { kind: "self" } }] }],
    }));
    const BIO = stubAttachment({ id: "bio", attachesTo: { kind: "minionWithHighestPrintedHp" }, abilities: [bio.ref] });
    const { deps, state } = setup({ cards: [BIO, BLAST], abilities: [bio, blastAbility], encounter: [...copies(THUG.id, 20), BIO.id] });
  ...
  it("'When attached enemy would attack, discard Webbed Up instead. Then, stun that enemy.'", () => {
    const webbed = stubAbility("webbed-up", def({
    ...
    // "Attach to an enemy."
    const WEBBED: AnyCard = { ...stubUpgrade({ id: "webbed", cost: 0, abilities: [webbed.ref] }), attachesTo: { kind: "enemy" } };
  ...
  it("'When attached minion is defeated, remove 3 threat from a scheme' (Spider-Tracer) — the defeat interrupt sees the host in play", () => {
    const tracer = stubAbility("tracer", def({
      trigger: { kind: "interrupt", forced: true, on: { on: "characterDefeated", targetIs: { hostOfSelf: true } } },
      effects: [{ kind: "removeThreat", target: { kind: "mainScheme" }, amount: { kind: "const", value: 3 } }],
    }));
    // "Attach to a minion."
    const TRACER: AnyCard = { ...stubUpgrade({ id: "tracer", cost: 0, abilities: [tracer.ref] }), attachesTo: { kind: "minion" } };
    const { deps, state } = setup({ cards: [TRACER, BLAST], abilities: [tracer, blastAbility], encounter: copies(THUG.id, 20) });
    const roundTwo = settle(runWith(deps, state, endTurn), undefined, deps);
    const [thug] = minionIn(roundTwo) as [InstanceId];
...
describe("cancelling a revealed encounter card (RRG 'Cancel', 'Surge')", () => {
  it("'cancel its When Revealed effects' (Enhanced Spider-Sense) also cancels surge", () => {
    const bomb = stubAbility("bomb", def({ trigger: { kind: "whenRevealed" }, effects: [{ kind: "placeThreat", target: { kind: "mainScheme" }, amount: { kind: "const", value: 2 } }] }));
    const BOMB = stubTreachery({ id: "bomb", boostIcons: 0, keywords: [{ name: "surge" }], abilities: [bomb.ref] });
    const sense = stubAbility("spider-sense", def({
      trigger: { kind: "interrupt", forced: false, form: "hero", on: { on: "encounterCardRevealing", targetIs: { categories: ["treachery"] } } },
  ...
  it("'exhaust Black Widow and spend a [mental] resource → cancel the effects of that card and discard it; reveal another'", () => {
    const widow = stubAbility("widow", def({
