...
const NEMESIS_SCHEME = stubSideScheme({
  id: "nemesis-scheme",
  startingThreat: 3,
  boostIcons: 0,
  encounterSetIds: ["hero-nemesis"],
});
const NEMESIS_TREACHERY: AnyCard = {
  ...stubTreachery({ id: "nemesis-treachery", boostIcons: 0, encounterSetIds: ["hero-nemesis"] }),
  quantityInSet: 2,
};
...
const idsOf = (state: GameState, ids: readonly InstanceId[], card: AnyCard) =>
  ids.filter((id) => state.instances[id]?.cardId === card.id);
...
function game(
  options: {
    ...
    scheme?: ReturnType<typeof stubMainScheme>;
    start?: number;
    last?: number;
...
describe("setup completeness (RRG Appendix II, 'Obligation', 'Nemesis Encounter Set')", () => {
  ...
  it("standard/expert villain stages: the game is won when the last stage used is defeated", () => {
    const three = stubVillain({
      id: "villain",
      stages: [
        { hp: flat(3), atk: 0, sch: 0 },
        { hp: flat(3), atk: 0, sch: 0 },
        { hp: flat(3), atk: 0, sch: 0 },
      ],
    });
    ...
    const standard = createGame(
      {
        seed: 5,
        cards: [...DEFAULT_CARDS, three, SCHEME, BLANK, KICK],
        villainCardId: three.id,
        mainSchemeCardId: SCHEME.id,
        encounterDeck: copies(BLANK.id),
        players: [{ identityCardId: HERO.id, deck: copies(KICK.id, 20) }],
    ...
    for (let i = 0; i < 2 && !state.outcome; i++) {
      ...
      state = runWith(deps, state, {
        type: "playCard",
        playerId: p1,
        cardInstanceId: kickId,
        payment: [],
  ...
  it("expert: the starting stage's When Revealed resolves at setup (Rhino II); a new stage brings Toughness and its When Revealed (Rhino III)", () => {
        ...
        effects: [
          {
            kind: "selectCards",
            slot: "b",
            cards: { kind: "encounter", zones: ["deck", "discard"], filter: { name: "breakin" } },
          },
    ...
    const rhinoIII = stubAbility(
      "rhino-iii",
      def({
        trigger: { kind: "whenRevealed" },
        effects: [{ kind: "giveStatus", target: { kind: "each", query: { categories: ["hero"] } }, status: "stunned" }],
      }),
    );
    ...
    const { deps, state } = game({
      cards: [BREAKIN, KICK],
      abilities: [rhinoII, rhinoIII, kick],
      villain: rhino,
      start: 1,
      encounter: [BREAKIN.id, ...copies(BLANK.id, 10)],
      deck: copies(KICK.id, 2),
    });
    ...
    const withKick: GameState = {
      ...state,
      players: state.players.map((p) => ({
        ...p,
        deck: p.deck.filter((id) => id !== kickId),
        hand: [...p.hand.filter((id) => id !== kickId), kickId],
      })),
    };
    const after = runWith(deps, withKick, toHero(), {
      type: "playCard",
      playerId: p1,
      cardInstanceId: kickId,
      payment: [],
...
describe("obligations (RRG 'Obligation', 'Reveal')", () => {
      ...
      options: [
        {
          ...
          effects: [
            { kind: "exhaust", target: { kind: "identityOf", player: you } },
        ...
        },
        {
          ...
          effects: [
            { kind: "discardFromHand", player: you, amount: { kind: "const", value: 1 }, random: true },
  ...
  it("an obligation whose hero isn't in the game is removed from the game and another card is revealed", () => {
    const orphan = stubAbility(
      "orphan",
      def({
        trigger: { kind: "whenRevealed" },
        effects: [{ kind: "placeThreat", target: { kind: "mainScheme" }, amount: { kind: "const", value: 9 } }],
      }),
    );
    const ORPHAN = stubObligation({ id: "absent-obligation", abilities: [orphan.ref] });
    const ABSENT = stubIdentity({
      id: "absent",
      hp: 10,
      atk: 1,
      thw: 1,
    ...
    const { deps, state } = game({
      cards: [ORPHAN, ABSENT],
      abilities: [orphan],
      encounter: [ORPHAN.id, ...copies(BLANK.id, 10)],
    });
...
describe("encounter-deck searches and set-aside cards", () => {
  it("'Discard cards from the encounter deck until a minion is discarded. Put it into play engaged with the first player' (1B When Revealed at setup)", () => {
    ...
    const scheme = stubMainScheme({
      id: "scheme",
      stages: [
        { startingThreat: flat(5), targetThreat: flat(40), acceleration: flat(0), abilities: [underground.ref] },
      ],
    });
    const { state } = game({
      cards: [THUG],
      ...
      scheme,
      encounter: [THUG.id, ...copies(BLANK.id, 5)],
    });
  ...
  it("Shadow of the Past: reveal your set-aside nemesis minion and side scheme; shuffle the rest of the set into the encounter deck", () => {
        ...
        effects: [
          {
            kind: "selectCards",
            slot: "nm",
            cards: { kind: "setAside", player: you, filter: { categories: ["minion"] } },
          },
          { kind: "revealCard", cards: { kind: "slot", slot: "nm" }, player: you },
          {
            kind: "selectCards",
            slot: "ns",
            cards: { kind: "setAside", player: you, filter: { categories: ["sideScheme"] } },
          },
          { kind: "revealCard", cards: { kind: "slot", slot: "ns" }, player: you },
          { kind: "moveCards", cards: { kind: "setAside", player: you }, to: "encounterDeckShuffle" },
          {
            kind: "if",
            condition: { kind: "not", of: { kind: "varAtLeast", name: "nm.count", amount: 1 } },
            then: [{ kind: "gainSurge" }],
          },
        ],
      }),
    ...
    const { deps, state } = game({
      cards: [SHADOW, ...NEMESIS],
      abilities: [shadow],
      encounter: [SHADOW.id, ...copies(BLANK.id, 10)],
    });
    ...
    const treacheries = Object.values(after.instances)
      .filter((i) => i.cardId === NEMESIS_TREACHERY.id)
      .map((i) => i.instanceId);
    expect(
      treacheries.every(
        (id) => activeEncounterDeck(after).deck.includes(id) || activeEncounterDeck(after).discard.includes(id),
      ),
    ).toBe(true);
  });

  it("Highway Robbery: cards placed facedown under the scheme (tucked, out of play) return to hand when it is defeated", () => {
    const robbery = stubAbility(
      "robbery-revealed",
      def({
        trigger: { kind: "whenRevealed" },
        effects: [
    ...
    const giveBack = stubAbility(
      "robbery-defeated",
      def({
        trigger: { kind: "whenDefeated" },
        effects: [{ kind: "moveCards", cards: { kind: "tucked", under: { kind: "self" } }, to: "hand" }],
      }),
    );
    const ROBBERY = stubSideScheme({
      id: "robbery",
      startingThreat: 1,
      boostIcons: 0,
      abilities: [robbery.ref, giveBack.ref],
    });
    const { deps, state } = game({
      cards: [ROBBERY],
      abilities: [robbery, giveBack],
      encounter: [ROBBERY.id, ...copies(BLANK.id, 10)],
    });
    ...
    const thwarted = runWith(deps, roundTwo, toHero(), {
      type: "basicThwart",
      playerId: p1,
      thwarterInstanceId: mustPlayer(roundTwo, p1).identity.instanceId,
      schemeInstanceId: robberyId,
    });
...
describe("damage assignment and hand discards", () => {
  it("'Assign X damage among heroes and allies' (Explosion) — one point at a time", () => {
    ...
    const state = newGame({
      villain: VILLAIN,
      mainScheme: SCHEME,
      extraCards: [EXPLOSION, PAL, BLANK],
      deck: [...copies(EXPLOSION.id, 3), ...copies(PAL.id, 3), ...copies(RESOURCE.id, 10)],
      encounterDeck: copies(BLANK.id),
      deps,
    });
  ...
  it("'Discard 1 card at random from each player's hand; 1 threat for each different resource type discarded' (The Vulture's Plans)", () => {
    const plans = stubAbility(
      ...
      def({
        ...
        effects: [
          {
            kind: "moveCards",
            cards: { kind: "zone", zone: "hand", player: { kind: "each" }, random: { kind: "const", value: 1 } },
            to: "discard",
            bind: "d",
          },
          {
            ...
            amount: { kind: "resourceTypes", cards: { kind: "slot", slot: "d" } },
          },
        ],
      }),
    );
    ...
    const state = newGame({
      ...
      villain: VILLAIN,
      mainScheme: SCHEME,
      extraCards: [PLANS, ENERGY, MENTAL, BLANK],
      deck: [...copies(PLANS.id, 2), ...copies(ENERGY.id, 4), ...copies(MENTAL.id, 4), ...copies(RESOURCE.id, 10)],
      encounterDeck: copies(BLANK.id),
      deps,
    });
    const find = (player: PlayerId, card: AnyCard) =>
      [...mustPlayer(state, player).hand, ...mustPlayer(state, player).deck].find(
        (id) => state.instances[id]?.cardId === card.id,
      ) as InstanceId;
    ...
    const rigged: GameState = {
      ...
      players: state.players.map((p) =>
        p.playerId === p1
          ? {
              ...p,
              hand: [plansId, energyId],
              deck: [...p.deck, ...p.hand].filter((id) => id !== plansId && id !== energyId),
            }
          : { ...p, hand: [mentalId], deck: [...p.deck, ...p.hand].filter((id) => id !== mentalId) },
      ),
    };
    const after = runWith(deps, rigged, {
      type: "playCard",
      playerId: p1,
      cardInstanceId: plansId,
      payment: [],
      attachToInstanceId: null,
    });
...
test("scenario-flow mechanics replay to an identical state", () => {
  ...
  const OBLIGATION = stubObligation({ id: "hero-obligation" });
  const { deps, state } = game({
    cards: [SHADOW, OBLIGATION, ...NEMESIS],
    abilities: [shadow],
    encounter: [SHADOW.id, ...copies(BLANK.id, 10)],
  });
