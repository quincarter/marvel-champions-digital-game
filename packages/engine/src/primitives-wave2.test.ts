...
import { stubAlly, stubEnvironment, stubEvent, stubIdentity, stubMinion, stubResource, stubSideScheme, stubSupport, stubTreachery } from "./testing/fixtures.js";
import { DEFAULT_CARDS, DEFAULT_DECK, defaultPick, giveCard, giveCards, MAIN_SCHEME, newGame, settle, TREACHERY, VILLAIN } from "./testing/scenario.js";
...
const QS_BASE = stubIdentity({ id: "qs", name: "Quicksilver", hp: 10, atk: 1, thw: 1, def: 1, rec: 1, heroHandSize: 5, alterEgoHandSize: 5 });
...
const ORDER: PlayerCard = { ...stubEvent({ id: "order-and-chaos", cost: 0, abilities: [ability("order.action", { kind: "action" })] }), keywords: [{ name: "teamUp", names: ["Quicksilver", "Scarlet Witch"] }] };
const VELOCITY: PlayerCard = { ...stubEvent({ id: "maximum-velocity", cost: 0, abilities: [ability("velocity.action", { kind: "action" })] }), playRestrictions: { maxPerPhase: 1 } };
...
const STINGER = stubAlly({
  ...
  thw: 1,
  hp: 3,
  abilities: [ability("stinger.constant", { kind: "constant", rules: [{ kind: "excludedFromAllyLimit", target: { self: true } }] })],
});
...
function game(): GameState {
  const state = newGame({
    ...
    deps,
    extraCards: CARDS,
    deck: [...DEFAULT_DECK, WITCH.id, ...copies(ORDER.id, 2), ...copies(VELOCITY.id, 2), ...copies(GRUNT.id, 4), STINGER.id],
  });
...
const play = (state: GameState, card: string, exclude: readonly InstanceId[] = []) => {
  ...
  const command: Command = { type: "playCard", playerId: p1, cardInstanceId: given.id, payment: [], attachToInstanceId: null };
...
describe("§3.6 counting boost icons (docs/phase7-wave2.md §3.6)", () => {
  ...
  const CHAOS = counting("chaos.interrupt", [{ kind: "replaceBoostCount", card: { kind: "identityOf", player: { kind: "controller" } } }]);
  const ONE_ICON = stubTreachery({ id: "one-icon", boostIcons: 1 });
  const withAbility = (ref: StubAbility) =>
    stubIdentity({ id: `wanda-${ref.ref.id}`, hp: 30, atk: 1, thw: 1, def: 1, rec: 1, heroHandSize: 5, alterEgoHandSize: 5, alterEgoAbilities: [ref.ref], heroAbilities: [ref.ref] });
  ...
  function schemeBoost(ability: StubAbility | null): Extract<GameEvent, { type: "schemeResolved" }> | undefined {
    ...
    const state = newGame({ identity, deps: localDeps, extraCards: [ONE_ICON], encounterDeck: Array.from({ length: 12 }, () => ONE_ICON.id) });
    ...
    while (result.ok && guard++ < 50) {
      ...
      const found = events.find((e): e is Extract<GameEvent, { type: "schemeResolved" }> => e.type === "schemeResolved");
      const choice = result.state.pendingChoice;
      if (found || !choice) return found;
      result = applyCommand(result.state, { type: "resolveChoice", playerId: choice.playerId, choiceId: choice.choiceId, selectedOptionIds: defaultPick(result.state) }, localDeps);
    }
    if (!result.ok) throw new Error(result.error.message);
    return undefined;
  }
...
describe("§3.7 division and repeated options (docs/phase7-wave2.md §3.7)", () => {
  ...
  const RETALIATOR = stubMinion({ id: "retaliator", atk: 0, sch: 0, hp: 9, boostIcons: 0, keywords: [{ name: "retaliate", value: 1 }] });
  const PLAIN = stubMinion({ id: "plain-minion", atk: 0, sch: 0, hp: 9, boostIcons: 0 });
  const STING = stubAbility("sting.action", {
    trigger: { kind: "action" },
    effects: [{ kind: "divide", what: "damage", amount: { kind: "const", value: 4 }, among: { categories: ["minion"] }, chooser: { kind: "controller" } }],
  });
    ...
    effects: [
      {
        ...
        allowRepeat: true,
        options: [
          { label: "A counter", effects: [{ kind: "addCounters", target: { kind: "identityOf", player: { kind: "controller" } }, counterType: "a", amount: HALF }] },
          { label: "B counter", effects: [{ kind: "addCounters", target: { kind: "identityOf", player: { kind: "controller" } }, counterType: "b", amount: HALF }] },
        ],
      },
    ],
  });
  const GIANT_RULE = stubAbility("giant.constant", {
    trigger: { kind: "constant", rules: [{ kind: "divideBasicPower", power: "attack", target: { categories: ["identity"], controller: "you" } }] },
    effects: [],
  });
  const refs = [STING.ref, TWICE.ref, GIANT_RULE.ref];
  const WASP = stubIdentity({ id: "wasp", hp: 20, atk: 3, thw: 1, def: 1, rec: 1, heroHandSize: 5, alterEgoHandSize: 5, heroAbilities: refs, alterEgoAbilities: refs });
  ...
  function waspGame(): { readonly state: GameState; readonly minions: readonly InstanceId[] } {
    const start = newGame({ identity: WASP, deps: localDeps, extraCards: [RETALIATOR, PLAIN], encounterDeck: [RETALIATOR.id, PLAIN.id, ...Array.from({ length: 10 }, () => PLAIN.id)] });
    ...
    return {
      ...
      state: {
        ...state,
        encounterDecks: { ...state.encounterDecks, [deckId as string]: { ...piles, deck: piles.deck.filter((id) => !minions.includes(id)) } },
  ...
  const use = (state: GameState, ability: StubAbility): GameState => {
    const result = applyCommand(state, { type: "useAbility", playerId: p1, cardInstanceId: state.players[0]!.identity.instanceId, abilityId: ability.ref.id, payment: [] }, localDeps);
    if (!result.ok) throw new Error(result.error.message);
    return result.state;
  };
  const answer = (state: GameState, ids: readonly string[]): GameState => {
    const choice = state.pendingChoice!;
    const result = applyCommand(state, { type: "resolveChoice", playerId: choice.playerId, choiceId: choice.choiceId, selectedOptionIds: ids }, localDeps);
    if (!result.ok) throw new Error(result.error.message);
    return result.state;
  };
  ...
  it("a divided basic attack attacks each target, and each retaliate hits the attacker (FAQ 'Wasp (#1C)', p. 61)", () => {
    ...
    const bad = applyCommand(state, { type: "basicAttack", playerId: p1, attackerInstanceId: hero, targetInstanceId: a, divide: [{ targetInstanceId: a, amount: 1 }, { targetInstanceId: b, amount: 1 }] }, localDeps);
    expect(bad.ok ? null : bad.error.message).toBe("the shares must total 3");
    const result = applyCommand(state, { type: "basicAttack", playerId: p1, attackerInstanceId: hero, targetInstanceId: a, divide: [{ targetInstanceId: a, amount: 2 }, { targetInstanceId: b, amount: 1 }] }, localDeps);
    if (!result.ok) throw new Error(result.error.message);
  ...
  it("a basic attack is not divided without the rule", () => {
    ...
    const hero = state.players[0]!.identity.instanceId;
    const result = applyCommand(state, { type: "basicAttack", playerId: p1, attackerInstanceId: hero, targetInstanceId: a, divide: [{ targetInstanceId: a, amount: 2 }, { targetInstanceId: b, amount: 1 }] }, plainDeps);
...
describe("§3.8 payment: X, dash, overpaying, playing ignoring cost (docs/phase7-wave2.md §3.8)", () => {
  const identityCounters = (effects: string) => ({ kind: "addCounters" as const, target: { kind: "identityOf" as const, player: { kind: "controller" as const } }, counterType: effects, amount: { kind: "var" as const, name: effects } });
  const CYCLONE_ACTION = stubAbility("cyclone.action", { trigger: { kind: "action" }, effects: [identityCounters("x")] });
  const CYCLONE: PlayerCard = { ...stubEvent({ id: "speed-cyclone", cost: 0, abilities: [CYCLONE_ACTION.ref] }), specialCost: "X" };
  ...
  const OVERPAY_ACTION = stubAbility("overpay.action", {
    trigger: { kind: "action" },
    effects: [
      { kind: "addCounters", target: { kind: "identityOf", player: { kind: "controller" } }, counterType: "over", amount: { kind: "var", name: "overpaid.total" } },
      { kind: "addCounters", target: { kind: "identityOf", player: { kind: "controller" } }, counterType: "overEnergy", amount: { kind: "var", name: "overpaid.energy" } },
    ],
  });
  const OVERPAY = stubEvent({ id: "overpay", cost: 1, abilities: [OVERPAY_ACTION.ref] });
  const MAGIC_ACTION = stubAbility("chaos-magic.action", { trigger: { kind: "action" }, effects: [{ kind: "playFromHand", player: { kind: "controller" }, ignoreCost: true }] });
  ...
  const REQUIRED: PlayerCard = { ...stubAlly({ id: "required", cost: 5, atk: 1, thw: 1, hp: 3 }), keywords: [{ name: "requirement", resources: { mental: 1 } }] };
  ...
  const payGame = (): GameState =>
    newGame({ deps: payDeps, extraCards: cards, deck: [...DEFAULT_DECK, ...cards.flatMap((card) => copies(card.id, 3))] });
  const counters = (state: GameState) => mustInstance(state, state.players[0]!.identity.instanceId).counters;
  const playWith = (state: GameState, card: string, payWith: readonly string[], extra: Partial<Extract<Command, { type: "playCard" }>> = {}) => {
    let current = state;
    ...
    const command: Command = { type: "playCard", playerId: p1, cardInstanceId: given.id, payment: paid.map((id) => ({ fromHand: id })), attachToInstanceId: null, ...extra };
  ...
  it("'Play a card from your hand, ignoring its resource cost' plays it with nothing paid, but never a Requirement card (RRG 1.8 'Ignore', p. 23)", () => {
    const { state: withCards, ids: [pricey, required] } = giveCards(payGame(), p1, PRICEY.id, REQUIRED.id);
    const magic = giveCard(withCards, p1, MAGIC.id);
    const cast = applyCommand(magic.state, { type: "playCard", playerId: p1, cardInstanceId: magic.id, payment: [], attachToInstanceId: null }, payDeps);
    ...
    const resolved = applyCommand(cast.state, { type: "resolveChoice", playerId: p1, choiceId: choice.choiceId, selectedOptionIds: [pricey!] }, payDeps);
    if (!resolved.ok) throw new Error(resolved.error.message);
    const state = settle(resolved.state, defaultPick, payDeps);
    expect(mustPlayer(state, p1).playArea).toContain(pricey);
    expect(resolved.events).toContainEqual(expect.objectContaining({ type: "cardPlayed", instanceId: pricey, resourcesPaid: 0 }));
  });
});
...
describe("§3.9 allies printed with 0 hit points (docs/phase7-wave2.md §3.9)", () => {
  ...
  const PYM_HP = stubAbility("pym.constant", {
    trigger: { kind: "constant", modifiers: [{ stat: "hp", amount: { kind: "counters", of: { kind: "self" }, counterType: "pym" }, target: { self: true } }] },
    effects: [],
  });
  const PYM_ENTERS = stubAbility("pym.enters", {
    trigger: { kind: "response", forced: true, on: { on: "cardEntersPlay", selfIs: "source" } },
    effects: [{ kind: "addCounters", target: { kind: "self" }, counterType: "pym", amount: { kind: "var", name: "overpaid.total" } }],
  });
  const ANT_ALLY = stubAlly({ id: "ant-ally", cost: 1, atk: 1, thw: 1, hp: 0, abilities: [PYM_HP.ref, PYM_ENTERS.ref] });
  const antDeps = depsOf(PYM_HP, PYM_ENTERS);
  const antGame = (): GameState => newGame({ deps: antDeps, extraCards: [ANT_ALLY], deck: [...DEFAULT_DECK, ANT_ALLY.id] });
  const playAnt = (resources: number) => {
    ...
    const result = applyCommand(ant.state, { type: "playCard", playerId: p1, cardInstanceId: ant.id, payment: paid.map((id) => ({ fromHand: id })), attachToInstanceId: null }, antDeps);
    if (!result.ok) throw new Error(result.error.message);
...
describe("§3.10 cards under and on other cards; owning a scenario card (docs/phase7-wave2.md §3.10)", () => {
  ...
  const ARROW = stubEvent({ id: "arrow", cost: 0, abilities: [ability("arrow.action", { kind: "action" }, [{ kind: "addCounters", target: { kind: "identityOf", player: { kind: "controller" } }, counterType: "arrow", amount: { kind: "const", value: 1 } }])] });
  const QUIVER = stubSupport({ id: "quiver", cost: 0, abilities: [ability("quiver.constant", { kind: "constant", playableAttachments: { categories: ["event"] } })] });
  const CAPTIVE_ALLY: AnyCard = { ...stubAlly({ id: "captive-ally", cost: 0, atk: 1, thw: 1, hp: 3 }), aspect: "none", specificTo: { kind: "scenario", encounterSetId: encounterSetId("taskmaster") } };
  // Test tools on the identity: tuck the ally in play under the villain, return it, arm the quiver, take the captive.
  const TUCK = ability("tuck.action", { kind: "action" }, [{ kind: "tuckCards", cards: { kind: "ref", ref: { kind: "each", query: { categories: ["ally"] } } }, under: { kind: "villain" } }]);
  const RETURN = ability("return.action", { kind: "action" }, [{ kind: "moveCards", cards: { kind: "tucked", under: { kind: "villain" } }, to: "hand" }]);
  const ARM = ability("arm.action", { kind: "action" }, [
    { kind: "chooseCards", slot: "arrow", from: { kind: "zone", zone: "deck", player: { kind: "controller" }, filter: { name: "arrow" } }, chooser: { kind: "controller" }, min: 1, max: 1 },
    { kind: "attach", card: { kind: "slot", slot: "arrow" }, to: { kind: "each", query: { name: "quiver" } } },
  ]);
  const TAKE = ability("take.action", { kind: "action" }, [{ kind: "takeIntoHand", cards: { kind: "encounterSetAside", filter: { name: "captive-ally" } }, player: { kind: "controller" } }]);
  const tools = [TUCK, RETURN, ARM, TAKE];
  const HAWK = stubIdentity({ id: "hawk", hp: 30, atk: 1, thw: 1, def: 1, rec: 1, heroHandSize: 5, alterEgoHandSize: 5, heroAbilities: tools, alterEgoAbilities: tools });
  ...
  const tool = (state: GameState, ref: { readonly id: string }): GameState =>
    run(state, { type: "useAbility", playerId: p1, cardInstanceId: state.players[0]!.identity.instanceId, abilityId: ref.id as never, payment: [] });
  const playFromHand = (state: GameState, card: string): { readonly state: GameState; readonly id: InstanceId } => {
    ...
    return { id: given.id, state: run(given.state, { type: "playCard", playerId: p1, cardInstanceId: given.id, payment: [], attachToInstanceId: null }) };
  };
  ...
  it("an event attached to Hawkeye's Quiver can be played from there, as if from hand", () => {
    ...
    const shot = run(armed, { type: "playCard", playerId: p1, cardInstanceId: arrow.instanceId, payment: [], attachToInstanceId: null });
  ...
  it("a scenario card taken into a player's hand becomes theirs, so it is discarded to their pile (RRG 1.8 'Ownership and Control', p. 31)", () => {
    ...
    const played = run(taken, { type: "playCard", playerId: p1, cardInstanceId: captive, payment: [], attachToInstanceId: null });
...
describe("§3.11 new trigger events and rules (docs/phase7-wave2.md §3.11)", () => {
  const counter = (type: string): EffectSpec => ({ kind: "addCounters", target: { kind: "identityOf", player: { kind: "controller" } }, counterType: type, amount: { kind: "const", value: 1 } });
  const USED = stubAbility("used.response", { trigger: { kind: "response", forced: true, on: { on: "basicPowerUsed", playerIs: "controller" } }, effects: [counter("used")] });
  const MIMIC = stubAbility("mimic.constant", { trigger: { kind: "constant", traitGrants: [{ traitsOf: { categories: ["environment"] }, target: { self: true } }] }, effects: [] });
  ...
  const PIETRO = stubIdentity({ id: "pietro", hp: 30, atk: 3, thw: 1, def: 1, rec: 1, heroHandSize: 5, alterEgoHandSize: 5, heroAbilities: tools, alterEgoAbilities: tools });
  const ASSAULT_SCHEME = { ...stubSideScheme({ id: "assaulted", startingThreat: 5, keywords: [{ name: "assault" }] }) };
  const RED_HOUSE = stubSideScheme({ id: "red-house", startingThreat: 5, abilities: [ability("red-house.constant", { kind: "constant", rules: [{ kind: "thwartWithAtk", scheme: { self: true } }] })] });
  const PORTAL = stubSideScheme({ id: "portal", startingThreat: 1, abilities: [ability("portal.constant", { kind: "constant", rules: [{ kind: "defeatedIntoEncounterDeck", target: { self: true } }] })] });
  const LAB = stubEnvironment({ id: "lab", traits: [trait("Laboratory")] });
  const DEPOWER = stubSupport({ id: "depower", cost: 0, abilities: [ability("depower.constant", { kind: "constant", rules: [{ kind: "cannotPlay", player: { kind: "controller" }, cards: { categories: ["ally"] } }] })] });
  const LOCKED = stubSupport({
    id: "locked",
    cost: 0,
    abilities: [ability("locked.constant", { kind: "constant", rules: [{ kind: "cannotTriggerActions", on: { self: true } }] }), ability("locked.action", { kind: "action" }, [counter("locked")])],
  });
  ...
  function ruleGame(inPlay: readonly AnyCard[]): GameState {
    const start = newGame({ identity: PIETRO, deps: ruleDeps, extraCards: [...encounter, DEPOWER, LOCKED, HELPER], encounterDeck: [...encounter.map((c) => c.id), ...copies(TREACHERY.id, 10)], deck: [...DEFAULT_DECK, DEPOWER.id, LOCKED.id, HELPER.id, HELPER.id] });
    ...
    for (const card of inPlay) {
      ...
      state = {
        ...state,
        encounterDecks: { ...state.encounterDecks, [deckId as string]: { ...piles, deck: piles.deck.filter((x) => x !== id) } },
  ...
  const idOf = (state: GameState, card: AnyCard): InstanceId => Object.values(state.instances).find((i) => i.cardId === card.id && state.villainArea.includes(i.instanceId))!.instanceId;
  ...
  it("'after you use a basic power' follows a basic thwart, and the Assault keyword thwarts with ATK (RRG 1.8 'Assault', p. 8)", () => {
    ...
    const done = run(state, { type: "basicThwart", playerId: p1, thwarterInstanceId: hero(state), schemeInstanceId: scheme });
  ...
  it("a confused thwart is not a basic power used (FAQ 'Quicksilver (#1A)', p. 61)", () => {
    ...
    const id = hero(start);
    const confused: GameState = { ...start, instances: { ...start.instances, [id]: { ...mustInstance(start, id), statuses: { stunned: 0, confused: 1, tough: 0 } } } };
    const done = run(confused, { type: "basicThwart", playerId: p1, thwarterInstanceId: id, schemeInstanceId: idOf(confused, ASSAULT_SCHEME) });
  ...
  it("'they may use their ATK instead of their THW' is the player's choice; without the rule it is refused", () => {
    ...
    const withAtk = run(state, { type: "basicThwart", playerId: p1, thwarterInstanceId: hero(state), schemeInstanceId: house, useAtk: true });
    expect(withAtk.ok && mustInstance(withAtk.state, house).threat).toBe(2);
    const refused = run(state, { type: "basicThwart", playerId: p1, thwarterInstanceId: hero(state), schemeInstanceId: idOf(state, PORTAL), useAtk: true });
  ...
  it("a defeated side scheme with the rule is shuffled into the encounter deck instead of discarded (Time Portal)", () => {
    ...
    const done = run(state, { type: "basicThwart", playerId: p1, thwarterInstanceId: hero(state), schemeInstanceId: portal });
    if (!done.ok) throw new Error(done.error.message);
  ...
  it("'You cannot play …' and 'cannot trigger … Action abilities' block the play and the action", () => {
    ...
    const played = run(depower.state, { type: "playCard", playerId: p1, cardInstanceId: depower.id, payment: [], attachToInstanceId: null });
    if (!played.ok) throw new Error(played.error.message);
    state = played.state;
    const helper = giveCard(state, p1, HELPER.id);
    const blocked = run(helper.state, { type: "playCard", playerId: p1, cardInstanceId: helper.id, payment: [], attachToInstanceId: null });
    ...
    const lockedIn = run(locked.state, { type: "playCard", playerId: p1, cardInstanceId: locked.id, payment: [], attachToInstanceId: null });
    if (!lockedIn.ok) throw new Error(lockedIn.error.message);
    const tried = run(lockedIn.state, { type: "useAbility", playerId: p1, cardInstanceId: locked.id, abilityId: "locked.action" as never, payment: [] });
  ...
  it("'When attached character would ready, … instead' replaces the ready (Frozen in Time)", () => {
    ...
    const played = run(helper.state, { type: "playCard", playerId: p1, cardInstanceId: helper.id, payment: [], attachToInstanceId: null });
    if (!played.ok) throw new Error(played.error.message);
    const exhausted: GameState = { ...played.state, instances: { ...played.state.instances, [helper.id]: { ...mustInstance(played.state, helper.id), exhausted: true } } };
...
describe("§3.12 selection vocabulary (docs/phase7-wave2.md §3.12)", () => {
  it("'1 random set-aside Captive ally' picks one of the matching set-aside cards from the seeded RNG", () => {
    ...
    const pick = (seed: number) => {
      ...
      const picked = selectCards(ctx, { kind: "encounterSetAside", filter: { trait: trait("Captive") }, random: { kind: "const", value: 1 } }, context);
