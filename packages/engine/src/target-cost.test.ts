...
import { ALLY, DEFAULT_DECK, RESOURCE, UPGRADE, expectOk, giveCards, newGame, payFor, runWith, settle } from "./testing/scenario.js";

const p1 = playerId("p1");
const p2 = playerId("p2");
const AVENGER = trait("Avenger");

const decoy: AbilityDefinition["effects"] = [{ kind: "dealDamage", target: { kind: "villain" }, amount: { kind: "const", value: 1 } }];
const actionWith = (id: string, definition: Omit<AbilityDefinition, "trigger">) => stubAbility(id, { trigger: { kind: "action" }, ...definition });
...
const STUCK_RULE = stubAbility("stuck.constant", { trigger: { kind: "constant", rules: [{ kind: "cannotLeavePlay", target: { self: true } }] }, effects: [] });
const STUCK = stubUpgrade({ id: "stuck", cost: 0, abilities: [STUCK_RULE.ref] });

// "Exhaust [the Shield] →" (Shield Block).
const BLOCK_ACTION = actionWith("block.action", { cost: { exhaustCards: { slot: "exhausted", query: { name: SHIELD.name }, min: 1, max: 1 } }, effects: decoy });
...
const TOSS_ACTION = actionWith("toss.action", {
  cost: { discardFromHand: { min: 0, bind: "x" }, returnToHand: { slot: "returned", query: { name: SHIELD.name }, min: 1, max: 1 } },
...
const TOSS_STUCK_ACTION = actionWith("toss-stuck.action", { cost: { returnToHand: { slot: "returned", query: { name: STUCK.name }, min: 1, max: 1 } }, effects: decoy });
...
const GENERATOR_RESOURCE = stubAbility("generator.resource", { trigger: { kind: "resource" }, cost: { exhaustSelf: true }, generates: { wild: 1 }, effects: [] });
const GENERATOR = stubSupport({ id: "generator", cost: 0, abilities: [GENERATOR_RESOURCE.ref] });
const OVERLOAD_ACTION = actionWith("overload.action", { cost: { exhaustCards: { slot: "exhausted", query: { categories: ["support"] }, min: 1, max: 1 } }, effects: decoy });
...
const DOUBLE_ACTION = actionWith("double.action", { cost: { exhaustSelf: true, exhaustCards: { slot: "exhausted", query: { categories: ["support"] }, min: 1, max: 1 } }, effects: decoy });
...
const reduction = (id: string, cardFilter?: { readonly categories: readonly ["ally"] }) =>
  actionWith(id, {
    effects: [{ kind: "reduceNextCardCost", player: { kind: "controller" }, amount: { kind: "const", value: 1 }, duration: "phase", ...(cardFilter ? { cardFilter } : {}) }],
  });
...
const TOWER_RULE = stubAbility("tower.constant", {
  trigger: {
    kind: "constant",
    rules: [{ kind: "allyLimit", amount: 1, while: { kind: "not", of: { kind: "exists", query: { categories: ["ally"], controller: "you", withoutTrait: AVENGER } } } }],
  },
  effects: [],
});
...
const DEMOLISH_ACTION = actionWith("demolish.action", { effects: [{ kind: "discardFromPlay", target: { kind: "named", name: TOWER.name } }] });
...
const MILL_ACTION = actionWith("mill.action", {
  effects: [
    { kind: "moveCards", cards: { kind: "encounter", zones: ["deck"], top: { kind: "const", value: 2 } }, to: "discard", bind: "milled" },
...
const EXTRA_CARDS = [SHIELD, STUCK, BLOCK, TOSS, TOSS_STUCK, STRENGTH, GENERATOR, OVERLOAD, DOUBLE, ALLY_DISCOUNT, ANY_DISCOUNT, TOWER, AVENGER_ALLY, PLAIN_ALLY, DEMOLISH, MILL];
const twice = (ids: readonly string[]) => [...ids, ...ids];
const DECK = [
  ...DEFAULT_DECK,
  ...twice([SHIELD.id, BLOCK.id, TOSS.id, ALLY_DISCOUNT.id, ANY_DISCOUNT.id]),
  ...[STUCK.id, TOSS_STUCK.id, STRENGTH.id, GENERATOR.id, OVERLOAD.id, DOUBLE.id, TOWER.id, DEMOLISH.id, MILL.id, PLAIN_ALLY.id],
  ...twice(twice([AVENGER_ALLY.id])),
];

const game = (players = 1) => newGame({ players, extraCards: EXTRA_CARDS, deck: DECK.map((id) => cardId(id)), deps: DEPS });
...
function inPlay(state: GameState, ...cards: readonly string[]): { readonly state: GameState; readonly ids: readonly InstanceId[] } {
...
describe("AbilityCost.exhaustCards (RRG 1.8 \"Cost\", pp. 13-14)", () => {
  it("a forced pick pays itself: the only matching card you control is exhausted with no costChoices", () => {
    ...
    const { state: dealt, ids: [block] } = giveCards(state, p1, BLOCK.id);
  ...
  it("with no matching card in play the ability can't be initiated, and legalActions doesn't offer it (p. 24, steps 3 and 5)", () => {
    const { state, ids: [block] } = giveCards(game(), p1, BLOCK.id);
  ...
  it("an exhausted match can't pay, and legalActions doesn't offer it", () => {
    ...
    const { state: dealt, ids: [block] } = giveCards(exhausted, p1, BLOCK.id);
  ...
  it("only cards the paying player controls can pay (p. 14; ruling June 25, 2026 #1)", () => {
    ...
    const { state: dealt, ids: [block] } = giveCards(theirs, p1, BLOCK.id);
  ...
  it("finds your own copy when another player's same-named card is earlier in play", () => {
    ...
    const { state: dealt, ids: [block] } = giveCards(theirsFirst, p1, BLOCK.id);
    const after = run(dealt, play(block!));
  ...
  it("a card outside play is not a legal pick, even if it matches the query", () => {
    ...
    const { state: dealt, ids: [strength, allyInHand] } = giveCards(state, p1, STRENGTH.id, ALLY.id);
  ...
  it("\"any number\" exhausts every chosen card and binds the count, but needs at least one (p. 14)", () => {
    ...
    const { state: dealt, ids: [strength] } = giveCards(state, p1, STRENGTH.id);
  ...
  it("one card can't pay two parts of a cost: the ability's own exhaust and the pick (p. 13)", () => {
    const { state, ids: [double] } = inPlay(game(), DOUBLE.id);
    const use: Command = { type: "useAbility", playerId: p1, cardInstanceId: double!, abilityId: DOUBLE_ACTION.ref.id, payment: [] };
  ...
  it("one card can't be exhausted both for a resource in the payment and for the cost (p. 13)", () => {
    const { state, ids: [generator] } = inPlay(game(), GENERATOR.id);
    const { state: dealt, ids: [overload] } = giveCards(state, p1, OVERLOAD.id, RESOURCE.id);
...
describe("AbilityCost.returnToHand", () => {
  it("returns the card from play to its owner's hand, off its host, and pays together with discard X", () => {
    const { state, ids: [shield] } = inPlay(game(), SHIELD.id);
    const host = mustInstance(state, shield!).attachedTo!;
    const { state: dealt, ids: [toss, spare] } = giveCards(state, p1, TOSS.id, ALLY.id);
    const handBefore = mustPlayer(dealt, p1).hand.length;
  ...
  it("when the return can't be paid, the discard isn't paid either: the ability can't be initiated (p. 24, step 5)", () => {
    const { state, ids: [toss, spare] } = giveCards(game(), p1, TOSS.id, ALLY.id);
  ...
  it("a card that cannot leave play can't be returned as a cost (RRG 1.8 \"Cannot\", p. 11)", () => {
    ...
    const { state: dealt, ids: [toss] } = giveCards(state, p1, TOSS_STUCK.id);
...
describe("reduceNextCardCost.cardFilter", () => {
  ...
  it("waits through a card it doesn't match, then the matching card uses and consumes it", () => {
    const { state, ids: [discount, upgrade, ally1, ally2] } = giveCards(game(), p1, ALLY_DISCOUNT.id, UPGRADE.id, ALLY.id, ALLY.id, RESOURCE.id, RESOURCE.id, RESOURCE.id);
  ...
  it("stacks with an unfiltered reduction: a matching card uses both, any other card only the unfiltered one", () => {
    const { state, ids: [allyDiscount, anyDiscount, upgrade, ally] } = giveCards(game(), p1, ALLY_DISCOUNT.id, ANY_DISCOUNT.id, UPGRADE.id, ALLY.id);
    const both = run(state, play(allyDiscount!), play(anyDiscount!));
    const afterUpgrade = run(both, play(upgrade!)); // 1 - 1 unfiltered = free
    expect(reductions(afterUpgrade).map((effect) => "cardFilter" in effect && effect.cardFilter !== undefined)).toEqual([true]);
    expectRefused(afterUpgrade, play(ally!), "insufficient_resources"); // 2 - 1 = 1

    const { state: again, ids: [allyDiscount2, anyDiscount2] } = giveCards(afterUpgrade, p1, ALLY_DISCOUNT.id, ANY_DISCOUNT.id);
  ...
  it("expires at the end of the phase when nothing matched it", () => {
    const { state, ids: [discount] } = giveCards(game(), p1, ALLY_DISCOUNT.id);
    const waiting = run(state, play(discount!));
...
describe("allyLimit.while (RRG 1.8 \"Ally Limit\", p. 7)", () => {
  const discardPrompt = (state: GameState) => (state.pendingChoice?.prompt.kind === "discardOverAllyLimit" ? state.pendingChoice : null);
  ...
  it("is read live when an ally enters play: a fourth Avenger fits, a non-Avenger drops the limit back to 3", () => {
    ...
    const { state: dealt, ids: [plain] } = giveCards(state, p1, PLAIN_ALLY.id);
  ...
  it("applies whenever the limit drops, not only when an ally enters: the Tower leaving play forces a discard", () => {
    const { state } = inPlay(game(), TOWER.id, AVENGER_ALLY.id, AVENGER_ALLY.id, AVENGER_ALLY.id, AVENGER_ALLY.id);
    const { state: dealt, ids: [demolish] } = giveCards(state, p1, DEMOLISH.id);
...
describe("value primitives", () => {
  const contextFor = (selfInstanceId: InstanceId | null, bindings: Record<string, readonly InstanceId[]> = {}): EffectContext => ({
    selfInstanceId,
    controllerId: p1,
    event: null,
    bindings,
  ...
  it("scaled.divide rounds the way the card says (RRG 1.8 p. 29 rounds up unless told otherwise)", () => {
    expect([halve(5, "down"), halve(5, "up"), halve(4, "down"), halve(1, "down"), halve(1, "up"), halve(0, "up")]).toEqual([2, 3, 2, 0, 1, 0]);
    expect(halve(5, "down", 0)).toBe(0); // an invalid divisor never produces NaN or Infinity
    const withTimes = resolveValue(game(), { kind: "scaled", value: { kind: "const", value: 7 }, divide: { by: 2, round: "down" }, times: 2, plus: 1 }, contextFor(null));
    expect(withTimes).toBe(7); // floor(7 / 2) * 2 + 1
  });

  it("maxPrintedCost as a value is re-read on every check", () => {
    const { state, ids: [quinjet] } = inPlay(game(), GENERATOR.id);
    const { state: dealt, ids: [ally] } = giveCards(state, p1, ALLY.id); // printed cost 2
    const query = { categories: ["ally"], maxPrintedCost: { kind: "counters", of: { kind: "self" }, counterType: "time" } } as const;
  ...
  it("countInRef counts matches among bound cards outside play, and an empty slot counts 0", () => {
    ...
    const count = (bindings: Record<string, readonly InstanceId[]>) =>
      resolveValue(state, { kind: "countInRef", cards: { kind: "slot", slot: "looked" }, query: { categories: ["treachery"] } }, contextFor(null, bindings));
  ...
  it("<bind>.boostIcons totals the boost icons (with modifiers) of every card a moveCards moved", () => {
    const { state, ids: [mill] } = giveCards(game(), p1, MILL.id);
