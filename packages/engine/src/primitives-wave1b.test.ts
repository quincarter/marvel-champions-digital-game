...
function setup(options: {
...
}): { deps: EngineDeps; state: GameState } {
  ...
  const state = newGame({
    ...
    deck: [
      ...cards.filter((card) => card.type !== "minion" && card.type !== "treachery").flatMap((card) => copies(card.id)),
...
describe("engine bugs from wave 1 scripting", () => {
  describe("`defended` opens an interrupt window (RRG 1.8 'Defend, Defense', p. 15; 'Interrupt', p. 25)", () => {
    ...
    const atDefense = () => {
      ...
      return {
        deps,
        eventId: given.ids[0] as InstanceId,
        state: settleUntil(runWith(deps, given.state, toHero, endTurn), "declareDefender", deps),
      };
    };
    ...
    it("offers 'when your hero defends' before the attack deals damage, and the DEF bonus reduces it", () => {
      ...
      expect(
        events.some(
          (e: GameEvent) => e.type === "triggerEvent" && e.event.kind === "defended" && e.phase === "initiated",
        ),
      ).toBe(true);
  ...
  it("an interrupt's 'that attack gains overkill' reaches a player's basic attack (Hulk Smash; RRG 1.8 'Overkill', p. 31)", () => {
    ...
    const smash = stubAbility(
      "smash",
      def({
        trigger: { kind: "interrupt", forced: true, on: { on: "attack", playerIs: "controller", attackKind: "basic" } },
        effects: [
          { kind: "modifyStatUntil", stat: "atk", amount: c(3), target: yourIdentity, until: "endOfAttack" },
  ...
  describe("a card's own triggered ability reads what was paid to play it (Valkyrie; RRG 1.8 'Cost', p. 13)", () => {
        ...
        effects: [
          {
            kind: "dealDamage",
            target: { kind: "villain" },
            amount: { kind: "conditional", if: { kind: "paidWith", resource: "energy" }, then: c(3), else: c(2) },
          },
        ],
      }),
    );
        ...
        effects: [
          {
            kind: "chooseCards",
            slot: "ally",
            from: { kind: "zone", zone: "hand", player: { kind: "controller" }, filter: { name: VALK.name } },
    ...
    it("put into play by another card's effect, it was not paid for", () => {
      ...
      const offered = settleUntil(
        runWith(deps, given.state, play(summonId, [{ fromHand: payId }])),
        "chooseTriggers",
        deps,
      );
      const after = settle(resolvePending(offered, [`${valkId}:valk-response`], deps), undefined, deps);
...
describe("wave 1 primitives", () => {
  it("TargetQuery.anyTrait matches a card with at least one of the traits, printed or granted (Morphogenetics)", () => {
    ...
    const wings = stubAbility(
      "wings",
      def({
        trigger: {
          kind: "constant",
          traitGrants: [{ trait: AERIAL, target: { categories: ["support"], anyTrait: [ATTACK, THWART] } }],
        },
        effects: [],
      }),
    );
    ...
    expect([a, t, p].map((id) => matchesQuery(inPlay, id, { anyTrait: [ATTACK, THWART] }, context))).toEqual([
      true,
      true,
      false,
    ]);
  ...
  it("ValueSpec sum adds its values, each read live (Generation Why?)", () => {
    ...
    expect(resolveValue(state, { kind: "sum", values: [{ kind: "sum", values: [c(1), c(1)] }, c(4)] }, context)).toBe(
      6,
    );
  ...
  describe("AbilityCost.discardRandomFromHand (Magic Crowbar, Ball and Chain, Bulldozer's Helmet)", () => {
    ...
    const crowbar = stubAbility(
      "crowbar",
      def({
        trigger: { kind: "action" },
        cost: { discardRandomFromHand: 1 },
        effects: [{ kind: "discardFromPlay", target: { kind: "self" } }],
      }),
    );
    const CROWBAR = stubSupport({ id: "crowbar", cost: 0, abilities: [crowbar.ref] });
    const pricey = stubAbility(
      "pricey",
      def({ trigger: { kind: "action" }, cost: { resources: 1, discardRandomFromHand: 1 }, effects: [] }),
    );
    ...
    const withHand = (state: GameState, keep: readonly InstanceId[]): GameState => ({
      ...
      players: state.players.map((p) =>
        p.playerId === p1 ? { ...p, hand: keep, deck: [...p.deck, ...p.hand.filter((id) => !keep.includes(id))] } : p,
      ),
    });
    const offered = (state: GameState, deps: EngineDeps, id: InstanceId): boolean => {
      ...
      return (
        result.kind === "turn" &&
        result.legal.some((entry) => entry.action.kind === "useAbility" && entry.action.instanceId === id)
      );
    };
    const handDiscards = (events: readonly GameEvent[]) =>
      events
        .filter((e): e is Extract<GameEvent, { type: "cardDiscardedFromHand" }> => e.type === "cardDiscardedFromHand")
        .map((e) => e.instanceId);
