...
export const CAP_CURATION: PackCuration = {
  ...
  pack: {
    ...
    releaseDateSource:
      'Hall of Heroes Captain America page (https://hallofheroeslcg.com/captain-america/): "Release date: December 20, 2019"',
  },
  outDir: "src/data/cap",
  exportPrefix: "CAP",
  ...
  corrections: [
    {
      ...
      reason:
        'MarvelCDB capitalizes "In"; the printed title lowercases it, confirmed on the deck\'s own printed decklist.',
  ...
  cardNotes: {
    "03025":
      'Honorary Avenger errata (RRG 1.8 p.65) added "Max 1 per character." on top of the printed "Play only if your identity has the Avenger trait." — data only carries the current playRestrictions shape (maxPerHost + requiresIdentityTrait); MarvelCDB\'s text already reflects the current wording.',
    "03015":
      "MarvelCDB's raw text has a stray closing </i> tag after \"Hero Action</b>\" with no matching open tag. No effect on the emitted card text — toPlainText strips all HTML tags unconditionally, so this never reaches a Correction's textReplace (which runs on the already-stripped text); recorded here only so the source anomaly is documented [evidence: raw; phase7 §1.12].",
  },
