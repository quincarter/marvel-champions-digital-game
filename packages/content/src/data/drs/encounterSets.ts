// GENERATED FILE — do not edit by hand.
// Source: MarvelCDB public API https://marvelcdb.com/api/public/cards/drs (fetched 2026-09-13; raw cache: packages/content/raw/marvelcdb/drs.json).
// Hand corrections / curated data: packages/content/scripts/marvelcdb/curation/drs.ts
// Regenerate: pnpm --filter @mc/content ingest -- --pack drs [--offline]

import { cardId, encounterSetId, setCode } from "../../schema/index.js";
import type { EncounterSet } from "../../schema/index.js";

export const DRS_ENCOUNTER_SETS: readonly EncounterSet[] = [
  {
    id: encounterSetId("doctor_strange_nemesis"),
    name: "Doctor Strange Nemesis",
    packCodes: [setCode("drs")],
    nemesisOfIdentityId: cardId("09001a"),
  },
];
