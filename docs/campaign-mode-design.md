...
| 12  | **Landed. Gate passed: MC21 fits.** Acceptance gate for "content-only": MC21's definition written against the frozen foundation, test-only (no `@mc/content` MC21 record exists yet — see the note below the table). Diff touches only `packages/cards` (plus this doc and a changie entry): no `packages/engine` or `packages/client` change was needed. One content-only DSL addition (`addAccelerationToken` gained an optional `target`). One genuine gap found and left as `// TODO(gap 1)`, not worked around — see the note below the table. | `ability-scripting-engineer` | `packages/cards/src/campaigns/mts.gate.test.ts`: shape assertions (citations, unique/prefixed ids, every referenced log field declared) plus the runner end to end (`createCampaignLog` → `resolveBetweenGames` → `startGameFromLog` → a hand-authored `CampaignGameResult` → `applyCampaignResult`), all five nodes, a loss and retry on Tower Defense, and a second walk proving Loki's Expert-Campaign-Only defeat block ends the campaign lost |
...
**One genuine gap**, left `// TODO(gap 1)` rather than worked around: no `ValueSpec` reads a player's deck size
(only `handCount`/`handSize` exist), so "each player discards the top half of their deck" (MC21 pp. 21, 25) cannot
be computed to pass as the `zone` `CardSelector`'s `top: ValueSpec`. Not campaign-mode-specific — this pack's own
card kit independently needs the same primitive (`docs/cards/by_pack/mts.md` prints an MC21 card's own When
Revealed, "removes the top half of their deck (rounded down) from the game") — for `game-rules-architect`: a
`ValueSpec` kind reading a player's deck count, mirroring `handCount`.
