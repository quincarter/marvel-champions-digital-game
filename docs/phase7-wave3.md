...
18. **A card bound by "discard until" that was the deck's last card (§4 Q15).** Teen Spirit (`msm` 05001b): "Discard cards from the top of your deck until you discard a Ms. Marvel card, then add that card to your hand." If the match is the last card of the deck, discarding it empties the deck, which resets at once, so the match is shuffled into the new deck before "add that card to your hand" resolves. The engine still moves it from the new deck to the hand, because the effect names the card itself (`moveCards` of the bound slot). The other reading is that the card has left the discard pile and can no longer be found there. The rulings file is silent.
