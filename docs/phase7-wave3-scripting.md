...
**Escape the Museum landed this session** (§1.1's ∞-hit-point Collector A2/B2 faces, the mode-labelled villain
...
known-but-unfixed correctness gap that isn't). **Next session on `gmw` should do Nebula or Ronan the Accuser**
(both need Power Stone and Galactic Artifacts isn't reached by any scenario yet either — see the table).
Menagerie Medley (16135–16137) is fully scripted (`gmw/museum.ts`) and Ship Command (16142–16148) is fully
scripted (`gmw/ship-command.ts`); both are shared by Escape the Museum. `MC_REFS_PACKS=gmw pnpm refs` is the
