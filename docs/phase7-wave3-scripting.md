...
| Galactic Artifacts modular set (16122–16130, five attachments and four side schemes)                                                                                                                                                                                                                                                                                                                                         | Scripted, one genuine primitive gap: `16125.the-poison-forced-interrupt` (module docblock in `gmw/galactic-artifacts.ts`: `TriggerEvent turnStarted` never opens an interrupt window, only a response one — an `isAnnouncement` event). Every other registered ref tested, see `gmw/galactic-artifacts.test.ts`. "Attach to the enemy/your identity with …" is data (`AttachmentCard.attachesTo`), already emitted; "Any player can do this" (Obedience Potion 16123, The Poison 16125) needed nothing new — an unowned attachment's Hero Action is already offered to every player (`legal.ts`'s `actionAbilities`).                                                                                                                                                                                                                                                         |
...
### `ron` (Kree Fanatic modular set): started, one genuine primitive gap
