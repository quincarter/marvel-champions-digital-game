...
2. **Depend on the engine primitives, don't reimplement them.** Villain attacks and scheme threat placement go through the same effect/trigger stack `game-rules-architect` owns — you're sequencing _when_ those primitives fire and _which_ targets/values they use, not building a parallel resolution system.
