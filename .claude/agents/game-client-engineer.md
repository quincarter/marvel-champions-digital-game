...
2. **Art is a build input, committed to the repo.** Card scans live in `assets/card-art/` (put there by the content scripts) and full-bleed pictures in `art/` (convention in `art/README.md`); both are tracked, by the owner's decision. `vite-card-art.ts` serves scans from disk in dev and bundles the pool's share into `dist/` at build — nothing is fetched at runtime, on web or in either native shell. Do not flag committed art as a policy gap, and do not add runtime fetching. A scan that is missing is a supported outcome: the card draws its generated frame, so art availability never blocks engine/UI work.
