...
Status: **Phases 0–3 complete; Phase 4 (client) in progress — the client spine has landed and a Core Set scenario is playable through the UI.** Client stack: pure Phaser 4 (4.2.1) with rexUI available behind an `Mc*` widget layer, no DOM UI framework (see Phase 4). Phase 2: Core Set ingestion, the §3 engine primitives, the `@mc/cards` ability DSL, all 233 Core ability scripts, `coreScenario()` builders, and real-content e2e games (spec: [docs/phase2-core-set.md](docs/phase2-core-set.md)). **Phase 3: the villain phase lives in `packages/engine/src/villain/`. Every decision the villain side leaves open is routed to the player the RRG names, and tagged with a `PendingChoice.authority`. `auditVillainPhases` independently re-checks each villain phase of a recorded game. 48 scripted games plus 3 passive-table games play to an outcome with clean audits (spec: [docs/phase3-encounter-ai.md](docs/phase3-encounter-ai.md)).** **Phase 4 so far: `legalActions` in the engine; and in `@mc/client` the `EngineHost` (worker + in-thread), the session store, the plain-TS view models (layout, board model, highlights, game log, villain-phase walkthrough) with 54 Vitest tests, the design-token module, the `Mc*` widget layer, and the Boot → Title/Setup → Board → Game Over scenes with the pending-choice overlay. A Rhino solo game plays through the UI: flip form, basic attack, play a card, resolve choices, with legal-move highlighting driven entirely by the engine.** Stack recorded in CLAUDE.md; monorepo scaffolded (`packages/engine`, `packages/content`, `packages/cards`, `packages/client`); card schema landed in `@mc/content` (see its README for the rules-shape decisions); `pnpm test` / `pnpm typecheck` green. Update the Status line and check off phases as work lands.
...
- [x] **Out-of-scope screens in the mocks** — confirmed out of scope and not built. Collection/deckbuilding beyond the preconstructed decks, and Campaign (marked locked in the mocks), stay out of Phase 4 unless re-scoped.
...
- **rexUI is installed and available but not yet used.** The `Mc*` widgets are drawn natively, which the design system makes cheap (borders and ground swaps, no gradients or shadows). rexUI earns its place at the pieces that are genuinely hard by hand — the virtualized log grid table, the Inspect scroll panel, the phone tab bar, `McTextInput` — and those are the next things to build on it. The widget layer is the seam that keeps that swap local.
...
- **Payment is not yet a player decision.** Playing a card uses the smallest payment the engine found (`LegalAction.example`), which `applyCommand` has already accepted. The Payment overlay, where the player picks what to spend, is the next piece of client work — `needsPayment` is already on every legal action.
- **The phone board has no tab bar yet.** `layout.ts` gives every tabbed zone the same rectangle (only one shows at a time) and the Board scene does not yet switch between them, so at phone widths the zones draw over each other. `McTabs` plus per-zone containers is the fix.
- **No animation pass yet.** `motion` tokens exist and `GameEvent`s are already folded per command, but the Board reconciles by redrawing rather than tweening. The instance-id → game-object map the stack decision calls for is the prerequisite.
- **No card art yet.** `assets/card-art/` and the `ArtRef` lookup are not wired; every card draws as the generated frame fallback, which is the designed behavior for a missing scan.
...
- [x] Board layout: player area(s), villain area, main scheme, side schemes, encounter deck/discard, each player's identity/hand/deck/discard/play area — legible at a glance the way the physical table is. Landed for the Long Table (desktop/tablet) layout; **the phone board's tab bar is still open** (see notes above).
- [ ] Card rendering (using licensed-for-personal-use art per the IP boundary in CLAUDE.md), zoom/inspect, legal-move highlighting, drag-and-drop or tap-to-target interaction for choosing targets/attachments/assignments. **Landed:** legal-move highlighting, tap-to-target for basic actions, tap-to-play for hand cards, and the generated-frame card rendering. **Open:** the Inspect overlay (full rules text), card art through `ArtRef`, drag-and-drop, and the Payment overlay.
- [ ] Animations/feedback for damage, threat, defeat, phase transitions — enough to make state changes readable, not spectacle for its own sake. **Open:** the Board redraws rather than tweening; needs the instance-id → game-object map.
- [ ] Accessibility pass: colorblind-safe indicators (damage/threat/keywords shouldn't rely on color alone), readable type sizes, keyboard/controller navigation if platform requires it. Scope per the stack decision above: keyboard and gamepad focus navigation, colorblind-safe indicators, minimum sizes and reduced motion. Screen readers are out of scope for the canvas client. **Landed:** icon-plus-text indicators (status pips carry their initial, stat tiles carry their label, the threat meter carries its numbers), the design's minimum type sizes and 44/52px touch targets, and a reduced-motion setting defaulting from `prefers-reduced-motion` (honoured by the selection ring). **Open:** keyboard and gamepad focus navigation with a visible focus ring.
- [ ] Exit criteria: a solo human player can play a full Core Set scenario against the AI villain from the Phase 3 engine, entirely through the UI, with no engine internals exposed. That covers one hero, or 1–4 heroes played multi-handed. **Partly met:** a Rhino solo game runs through the UI from setup to real turns (verified: mulligan, change form, basic attack, card plays, choice resolution, villain phase), and no engine internals are exposed. Playing a scenario *to an outcome* comfortably still wants the Payment overlay, the Inspect overlay, and the villain-phase walkthrough screen.
