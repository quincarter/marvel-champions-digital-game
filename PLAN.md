...
- **rexUI is installed and available but still unused.** The `Mc*` widgets — `McTabs` included — are drawn natively, which the design system makes cheap (borders and ground swaps, no gradients or shadows). rexUI still earns its place at the pieces that are genuinely hard by hand: the virtualized log grid table, the Inspect scroll panel, and `McTextInput`. The widget layer is the seam that keeps that swap local.
- **No card-movement tweens.** Beats make *what happened* readable; cards still appear in their new zone rather than travelling there. That needs the instance-id → game-object map the stack decision calls for, and is the remaining half of the animation pass.
- **Gamepad navigation.** The focus model is input-agnostic and tested; only the keyboard is wired to it.
- **The phone hand crowds at six cards.** `cardRow` shrinks to a floor and overlaps; the mock scrolls the hand horizontally with a "Fan out" affordance instead.
- **Abilities on cards in play are not reachable from the UI.** `legalActions` lists them and `paymentFor` prices them; the board has no control that triggers one.
