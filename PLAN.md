...
- **Running now (2026-09-13): seven `ability-scripting-engineer` agents in parallel,** one each for `msm`, `thor`, `bkw`, `drs`, `hlk`, `gob` and `twc`.
  ...
  - **Asked for by the user (2026-09-15): finish the skipped cards.** Triage of the 31 `KNOWN_SKIPPED` refs against what has since landed:
    ...
    - **Wave B scripting is running (`ability-scripting-engineer`, 2026-09-16),** the only agent. The main session confirmed typecheck was clean first. It covers the 19 `KNOWN_SKIPPED` refs. Teen Spirit (05001b) may stay skipped, since no player-deck `discardDeckUntil` was in the primitives batch. It writes only under `packages/cards/src/wave1/`, with no engine, content or client changes.
